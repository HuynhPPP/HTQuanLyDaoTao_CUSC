...
<section class="section">
    <div class="row border border-3 rounded-3 mt-5 text-center bg-white shadow">
        <div class="col my-5">
            ...
            <p class="text-secondary">Khu III, Đại học Cần Thơ – 01 Lý Tự Trọng, Tp. Cần Thơ – Tel: 0292.3731072 & Fax: 0292.3731071 – Email: cusc@ctu.edu.vn</p>
        </div>
        ...
        <table class="table table-bordered table-hover rounded shadow-sm">
            ...
            <tbody>
                ...
                foreach ($ngaynghis as $ngaynghi) {
                 ...
                 $getSubjectForDay = function(&$subjectOccurrences, $currentDate, &$totalHours, &$examDays, &$selfStudyDays, $addDaysSkippingWeekends, $holidayDates, &$examCounter) {
                        ...
                        if ($details['remaining'] > 0) {
                            ...
                            if ($details['remaining'] <= 0) {
                                ...
                                if (isset($details['lastSubject']) && $details['lastSubject']) {
                                    ...
                                    while (isset($holidayDates[$examDate->format('Y-m-d')]) || isset($selfStudyDays[$examDate->format('Y-m-d')])) {
                                        $examDate->addDay();
                                    }
                        ...
                        if ($currentDate->gte($startDate)) {
                            if (isset($examDays[$currentDate->format('Y-m-d')])) {
                                ...
                                if (isset($holidayDates[$currentDate->format('Y-m-d')])) {
                                    ...
                                    $subject = $getSubjectForDay($subjectOccurrences, $currentDate, $totalHours, $examDays, $selfStudyDays, $addDaysSkippingWeekends, $holidayDates, $examCounter);
