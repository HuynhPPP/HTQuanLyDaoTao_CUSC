...
@section('main-content')
    <div class="container my-5">
        <div class="row border border-3 rounded-3 mt-5 text-center bg-white shadow">
            <div class="col my-5">
                <img src="{{ asset('images/logo_CTU.png') }}" alt="logo" width="80" class="mb-3">
                <h5 class="fw-bold text-primary">TRUNG TÂM CÔNG NGHỆ PHẦN MỀM ĐẠI HỌC CẦN THƠ</h5>
                <h1 class="fw-bold">CANTHO UNIVERSITY SOFTWARE CENTER</h1>
                <p class="text-secondary">Khu III, Đại học Cần Thơ – 01 Lý Tự Trọng, Tp. Cần Thơ – Tel: 0292.3731072 & Fax:
                    0292.3731071 – Email: cusc@ctu.edu.vn</p>
            </div>
            <div class="text-center mb-3">
                <h1 class="fw-bold text-success">{{ $schedule->TenTKB }}</h1>
            </div>
            <div class="row justify-content-between">
                ...
                <div class="col-4 text-start">
                    <p class="m-0">Bắt đầu học từ ngày: {{ \Carbon\Carbon::parse($schedule->NgayHoc)->format('d/m/Y') }}
                    </p>
                    ...
                    @php
                        ...
                        foreach ($ngaytuhocs as $ngaytuhoc) {
                            ...
                            while ($selfStudyStart->lte($selfStudyEnd)) {
                                if (
                                    $selfStudyStart->dayOfWeek !== Carbon::SATURDAY &&
                        ...
                        foreach ($ngaynghis as $ngaynghi) {
                            ...
                            while ($holidayStart->lte($holidayEnd)) {
                                if (
                                    $holidayStart->dayOfWeek !== Carbon::SATURDAY &&
                                    $holidayStart->dayOfWeek !== Carbon::SUNDAY
                                ) {
                                                    ...
                                                    if (!isset($selfStudyDays[$selfStudyDate->format('Y-m-d')])) {
                                                        ...
                                                        foreach ($subjectOccurrences as $s => &$d) {
                                                            if ($d['first'] && $d['first']->eq($selfStudyDate)) {
                                                                $d['first'] = $addDaysSkippingWeekends(
                                                                    $selfStudyDate->copy(),
                                                                    1,
                                                                );
                                                                $d['last'] = $addDaysSkippingWeekends(
                                                                    $d['last']->copy(),
                                                                    1,
                                                                );
                                                            }
        ...
        <div class="d-flex justify-content-center mt-3 gap-2">
            <form id="deleteScheduleForm" action="{{ route('deleteSchedule', ['TenTKB' => $schedule->TenTKB]) }}"
                method="POST">
                @csrf
                @method('DELETE')
    ...
    <div class="position-fixed bottom-0 start-0 m-3">
        <div class="mb-2">
            <button type="button" class="btn btn-primary btn-lg rounded-circle" id="addTimeSlotButton"
                data-bs-toggle="modal" data-bs-target="#timeSlotModal">
                <i class="fas fa-clock"></i>
            </button>
        </div>
        <div class="mb-2">
            <button type="button" class="btn btn-primary btn-lg rounded-circle" id="addAbsenceButton"
                data-bs-toggle="modal" data-bs-target="#absenceModal">
                <i class="fas fa-plus"></i>
            </button>
        </div>
        <div class="mb-2">
            <button type="button" class="btn btn-primary btn-lg rounded-circle" id="addSelfStudy" data-bs-toggle="modal"
                data-bs-target="#SelfStudyModal">
                <i class="fa-brands fa-leanpub"></i>
            </button>
        </div>
        <div class="mb-2">
            <button type="button" class="btn btn-primary btn-lg rounded-circle" id="editTKB" data-bs-toggle="modal"
                ...
                <div class="modal-body">
                    <form id="timeSlotForm" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <select id="khunggio" class="form-select @error('khunggio') is-invalid @enderror"
                                name="khunggio">
    ...
    <div class="modal fade" id="SelfStudyModal" tabindex="-1" aria-labelledby="SelfStudyModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="SelfStudyForm" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <select id="ngaytuhoc" class="form-select @error('ngaytuhoc') is-invalid @enderror"
                                name="ngaytuhoc">
