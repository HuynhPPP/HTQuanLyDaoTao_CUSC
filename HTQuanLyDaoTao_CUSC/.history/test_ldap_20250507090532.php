...
$ldap_host = "10.0.0.2";
$ldap_port = 389;
$ldap_user = "CUSC\\htkhuong2003"; // Định dạng domain\username
$ldap_password = "P@ssW@rd2025!";

// Tạo connection string
$ldap_conn_string = "ldap://{$ldap_host}";  // Bỏ port trong connection string
echo "Đang kết nối đến: {$ldap_conn_string}\n";

// Kết nối đến LDAP server với timeout
$ds = ldap_connect($ldap_conn_string, $ldap_port);
if (!$ds) {
...
ldap_set_option($ds, LDAP_OPT_NETWORK_TIMEOUT, 10); // Timeout 10 giây
...
if (!ldap_bind($ds, $ldap_user, $ldap_password)) {
    ...
    die("LDAP bind thất bại:\n" .
        ...
        "4. Kiểm tra xem LDAP service có đang chạy trên server không\n");
}

echo "LDAP bind thành công!\n";
ldap_close($ds);
