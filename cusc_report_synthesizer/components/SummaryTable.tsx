                ...
                <table className="w-full border-collapse text-xs">
                    <thead className="bg-gray-200">
                        <tr>
                            ...
                            <th className="border border-slate-600 p-2 font-bold w-[10%] no-print">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.entries.map((entry, entryIndex) => (
                            <React.Fragment key={entry.id}>
                                ...
                                {entry.instructors.map((inst, instIndex) => (
                                    <tr key={inst.id} className="print-break-inside-avoid hover:bg-slate-50 group">
                                        <td className="border border-slate-500 p-1 text-center">{entry.instructors.slice(0, instIndex).length + 1}</td>
                                        ...
                                        <td className="border border-slate-500 p-0 text-center">
                                            <EditableCell value={(Number(inst.hours) || 0).toFixed(2)} onChange={v => handleInstructorChange(entry.id, inst.id, 'hours', Number(v))} type="number" />
                                        </td>
                                        <td className="border border-slate-500 p-0">
                                             <EditableCell value={inst.notes} onChange={v => handleInstructorChange(entry.id, inst.id, 'notes', v as string)} />
                                        </td>
