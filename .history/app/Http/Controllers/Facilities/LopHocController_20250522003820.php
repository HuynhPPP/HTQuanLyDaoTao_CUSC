...
class LopHocController extends Controller
{
    ...
    {
        try {
            // Kiểm tra sự tồn tại của lớp học và sinh viên
            $lophoc = lophoc::findOrFail($malop);
            $sinhvien = sinhvien::findOrFail($masv);

            // Xóa sinh viên khỏi lớp học
            danhsachsv::where('MaLop', $malop)
                ->where('MaSV', $masv)
                ->delete();
    ...
    public function addTeacherForm($maLop)
    {
        $lophoc = lophoc::findOrFail($maLop);
        $giaoviens = giaovien::all();
        $existingTeachers = $lophoc->giangViens->pluck('MaGV')->toArray();

        return view('quanly_cosovatchat.lophoc.add-teacher', compact('lophoc', 'giaoviens', 'existingTeachers'));
    }

    public function addTeacher(Request $request, $maLop)
    {
        $request->validate([
        ...
        // Kiểm tra xem giảng viên đã được phân công cho lớp này chưa
        if (!$lophoc->giangViens->contains($request->MaGV)) {
            $lophoc->giangViens()->attach($request->MaGV, [
                'NgayBatDau' => $request->NgayBatDau,
                'NgayKetThuc' => $request->NgayKetThuc,
                'GhiChu' => $request->GhiChu
            ]);
            return redirect()->route('lophoc.index', $maLop)
                ->with('success', 'Phân công giảng viên thành công.');
        }

        return redirect()->back()
            ->with('error', 'Giảng viên đã được phân công cho lớp này.');
    }
