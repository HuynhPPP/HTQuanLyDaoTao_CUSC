...
class MonHocController extends Controller
{
    ...
    public function storeTeacher(Request $request, $MaMH)
    {
        ...
        if (!empty($assignedClasses)) {
            ...
            return redirect()->route('monhoc.add-teacher', $MaMH)->with('warning', $message);
        }
    ...
    public function updateTeacherAssignment(Request $request, $MaMH, $maGV)
    {
        $validatedData = $request->validate([
            ...
            'MaGV.exists' => 'Giảng viên không tồn tại trong hệ thống.',
            'MaLop.*.exists' => 'Một trong các lớp học không tồn tại trong hệ thống.',
            'NgayBatDau.date' => 'Ngày bắt đầu không hợp lệ.',
            'NgayKetThuc.date' => 'Ngày kết thúc không hợp lệ.',
            'NgayKetThuc.after' => 'Ngày kết thúc phải sau ngày bắt đầu.'
        ]);
        ...
        // Kiểm tra xem phân công mới đã tồn tại chưa
        $existingAssignment = \DB::table('giangday')
            ->where('MaMH', $MaMH)
        ...
        if ($existingAssignment && $existingAssignment->MaGV != $maGV) {
            return redirect()->back()
                ->with('error', 'Phân công giảng dạy này đã tồn tại')
                ->withInput();
        }
        // Xoá các phân công cũ của giảng viên này trong môn học này
        \DB::table('giangday')
            ->where('MaMH', $MaMH)
            ->where('MaGV', $maGV)
            ->delete();
        ...
        foreach ($validatedData['MaLop'] as $maLop) {
            \DB::table('giangday')->update([
                'MaMH' => $MaMH,
