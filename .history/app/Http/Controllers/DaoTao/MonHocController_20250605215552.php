...
class MonHocController extends Controller
{
    ...
    public function addClassForm($maMH)
    {
        $monhoc = MonHoc::where('MaMH', $maMH)->firstOrFail();
        $giaoviens = giaovien::all();
        $existingTeachers = $monhoc->giangViens->pluck('MaGV')->toArray();
    ...
    public function assignClassForm($maMH)
    {
        $monhoc = MonHoc::where('MaMH', $maMH)->firstOrFail();
        $giaoviens = GiaoVien::all();
        $lops = Lop::all(); // Giả sử có model Lop

        // Lấy các phân công hiện tại
        $currentAssignments = GiangDay::where('MaMH', $maMH)->get();

        return view('quanly_daotao.monhoc.assign-class', compact('monhoc', 'giaoviens', 'lops', 'currentAssignments'));
    }
    public function storeClassAssignment(Request $request, $maMH)
    {
        ...
        // Kiểm tra xem đã tồn tại phân công chưa
        $existingAssignment = GiangDay::where('MaMH', $maMH)
            ->where('MaGV', $validatedData['MaGV'])
            ->where('MaLop', $validatedData['MaLop'])
            ->first();

        if ($existingAssignment) {
            return redirect()->back()->with('error', 'Phân công này đã tồn tại.');
        }
        ...
        GiangDay::create([
            ...
            'NgayBatDau' => $validatedData['NgayBatDau'] ?? null,
            'NgayKetThuc' => $validatedData['NgayKetThuc'] ?? null,
            'GhiChu' => $validatedData['GhiChu'] ?? null
        ]);

        return redirect()->route('monhoc.index')->with('success', 'Phân công giảng viên cho lớp thành công.');
    }
}