<?php

namespace App\Http\Controllers\DaoTao;

use App\Http\Controllers\Controller;
use App\Models\ChuongTrinh;
use App\Models\khoadaotao;
use App\Models\monhoc;
use App\Models\ChuongTrinhMonHoc;
use App\Models\HinhThucDanhGia;
use App\Models\TieuChiXepLoai;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Validator;

class ChuongTrinhDaoTaoController extends Controller
{
    public function index()
    {
        $chuongTrinhs = ChuongTrinh::get();
        return view('quanly_daotao.chuongtrinhdaotao.index', compact('chuongTrinhs'));
    }

    public function create()
    {
        $khoadaotaos = khoadaotao::all();
        return view('quanly_daotao.chuongtrinhdaotao.create', compact('khoadaotaos'));
    }
    // public function store(Request $request)
    // {
    //     $validator = Validator::make($request->all(), [
    //         'MaChuongTrinh' => 'required|unique:chuongtrinh|max:12',
    //         'TenChuongTrinh' => 'required',
    //         'PhienBan' => 'nullable|max:12',
    //         'NgayTrienKhaiPB' => 'nullable|date',
    //         'TenKhoaDaoTao' => 'required',
    //     ], [
    //         'MaChuongTrinh.required' => 'Mã chương trình không được để trống',
    //         'TenKhoaDaoTao.required' => 'Tên khoá đào tạo không được để trống',
    //         'TenChuongTrinh.required' => 'Tên chương trình đào tạo không được để trống',
    //         'MaChuongTrinh.unique' => 'Mã chương trình đã tồn tại trong hệ thống',
    //         'MaChuongTrinh.max' => 'Mã chương trình không được vượt quá 12 ký tự'
    //     ]);

    //     if ($validator->fails()) {
    //         return redirect()->back()
    //             ->withErrors($validator)
    //             ->withInput();
    //     }

    //     try {
    //         ChuongTrinh::create($request->all());
    //         return redirect()->route('chuongtrinh.index')
    //             ->with('success', 'Thêm chương trình đào tạo thành công');
    //     } catch (\Exception $e) {
    //         return redirect()->back()
    //             ->with('error', 'Có lỗi xảy ra: ' . $e->getMessage());
    //     }
    // }

    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'MaChuongTrinh' => 'required|unique:chuongtrinh|max:12',
            'TenChuongTrinh' => 'required',
            'PhienBan' => 'nullable|max:12',
            'NgayTrienKhaiPB' => 'nullable|date',
            'TenKhoaDaoTao' => 'required',
        ], [
            'MaChuongTrinh.required' => 'Mã chương trình không được để trống',
            'TenKhoaDaoTao.required' => 'Tên khoá đào tạo không được để trống',
            'TenChuongTrinh.required' => 'Tên chương trình đào tạo không được để trống',
            'MaChuongTrinh.unique' => 'Mã chương trình đã tồn tại trong hệ thống',
            'MaChuongTrinh.max' => 'Mã chương trình không được vượt quá 12 ký tự'
        ]);

        if ($validator->fails()) {
            return redirect()->back()
                ->withErrors($validator)
                ->withInput();
        }

        DB::beginTransaction();
        try {
            // Tạo chương trình đào tạo
            $chuongTrinh = ChuongTrinh::create($request->only([
                'MaChuongTrinh',
                'TenChuongTrinh',
                'PhienBan',
                'NgayTrienKhaiPB',
                'TenKhoaDaoTao'
            ]));

            // Lưu hình thức đánh giá
            if ($request->has('danhgia')) {
                foreach ($request->danhgia as $item) {
                    HinhThucDanhGia::create([
                        'MaChuongTrinh' => $chuongTrinh->MaChuongTrinh,
                        'HinhThuc' => $item['HinhThuc'] ?? null,
                        'TiLePhanTram' => $item['TiLePhanTram'] ?? null,
                        'SoBaiThi' => $item['SoBaiThi'] ?? null,
                        'DiemMoiBai' => $item['DiemMoiBai'] ?? null,
                        'ThoiGian' => $item['ThoiGian'] ?? null,
                        'DonViThoiGian' => $item['DonViThoiGian'] ?? null,
                    ]);
                }
            }

            // Lưu tiêu chí xếp loại
            if ($request->has('xeploai')) {
                foreach ($request->xeploai as $item) {
                    TieuChiXepLoai::create([
                        'MaChuongTrinh' => $chuongTrinh->MaChuongTrinh,
                        'XepLoai' => $item['XepLoai'] ?? null,
                        'DiemTu' => $item['DiemTu'] ?? null,
                        'DiemDen' => $item['DiemDen'] ?? null,
                    ]);
                }
            }

            DB::commit();
            return redirect()->route('chuongtrinh.index')
                ->with('success', 'Thêm chương trình đào tạo thành công');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()
                ->with('error', 'Có lỗi xảy ra: ' . $e->getMessage());
        }
    }
    public function edit($maChuongTrinh)
    {
        $chuongTrinh = ChuongTrinh::findOrFail($maChuongTrinh);
        $khoadaotaos = khoadaotao::all();
        return view('quanly_daotao.chuongtrinhdaotao.edit', compact('chuongTrinh', 'khoadaotaos'));
    }
    public function update(Request $request, $maChuongTrinh)
    {
        // dd($request->all());
        $validator = Validator::make($request->all(), [
            'MaChuongTrinh' => 'required',
            'TenChuongTrinh' => 'required',
            'PhienBan' => 'nullable|max:12',
            'NgayTrienKhaiPB' => 'nullable|date',
            'TenKhoaDaoTao' => 'required',
        ], messages: [
            'MaChuongTrinh.required' => 'Mã chương trình không được để trống',
            'TenKhoaDaoTao.required' => 'Tên khoá đào tạo không được để trống',
            'TenChuongTrinh.required' => 'Tên chương trình đào tạo không được để trống'
        ]);

        if ($validator->fails()) {
            return redirect()->back()
                ->withErrors($validator)
                ->withInput();
        }

        DB::beginTransaction();
        try {
            // Cập nhật thông tin chương trình
            $chuongTrinh = ChuongTrinh::findOrFail($maChuongTrinh);
            $chuongTrinh->update($request->only([
                'MaChuongTrinh',
                'TenChuongTrinh',
                'PhienBan',
                'NgayTrienKhaiPB',
                'TenKhoaDaoTao'
            ]));

            // Xóa các hình thức đánh giá cũ
            HinhThucDanhGia::where('MaChuongTrinh', $maChuongTrinh)->delete();

            // Lưu hình thức đánh giá mới
            if ($request->has('danhgia')) {
                foreach ($request->danhgia as $item) {
                    HinhThucDanhGia::create([
                        'MaChuongTrinh' => $maChuongTrinh,
                        'HinhThuc' => $item['HinhThuc'] ?? null,
                        'TiLePhanTram' => $item['TiLePhanTram'] ?? null,
                        'SoBaiThi' => $item['SoBaiThi'] ?? null,
                        'DiemMoiBai' => $item['DiemMoiBai'] ?? null,
                        'ThoiGian' => $item['ThoiGian'] ?? null,
                        'DonViThoiGian' => $item['DonViThoiGian'] ?? null,
                    ]);
                }
            }

            // Xóa các tiêu chí xếp loại cũ
            TieuChiXepLoai::where('MaChuongTrinh', $maChuongTrinh)->delete();

            // Lưu tiêu chí xếp loại mới
            if ($request->has('xeploai')) {
                foreach ($request->xeploai as $item) {
                    TieuChiXepLoai::create([
                        'MaChuongTrinh' => $maChuongTrinh,
                        'XepLoai' => $item['XepLoai'] ?? null,
                        'DiemTu' => $item['DiemTu'] ?? null,
                        'DiemDen' => $item['DiemDen'] ?? null,
                    ]);
                }
            }

            DB::commit();
            return redirect()->route('chuongtrinh.index')
                ->with('success', 'Cập nhật chương trình đào tạo thành công');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()
                ->with('error', 'Có lỗi xảy ra: ' . $e->getMessage());
        }
    }
    public function destroy($maChuongTrinh)
    {
        try {
            $chuongTrinh = ChuongTrinh::findOrFail($maChuongTrinh);
            $chuongTrinh->delete();
            return redirect()->route('chuongtrinh.index')
                ->with('success', 'Xóa chương trình đào tạo thành công');
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Không thể xóa chương trình đào tạo: ' . $e->getMessage());
        }
    }
    public function showMonHoc($maChuongTrinh)
    {
        $chuongTrinh = ChuongTrinh::findOrFail($maChuongTrinh);
        $monHocList = MonHoc::all();
        $monHocDaGan = ChuongTrinhMonHoc::where('MaChuongTrinh', $maChuongTrinh)->get();

        return view('quanly_daotao.chuongtrinhdaotao.monhoc', compact('chuongTrinh', 'monHocList', 'monHocDaGan'));
    }
    public function storeMonHoc(Request $request, $maChuongTrinh)
    {
        $validator = Validator::make($request->all(), [
            'MaMH' => 'required|array',
            'MaMH.*' => 'exists:monhoc,MaMH'
        ], [
            'MaMH.required' => 'Vui lòng chọn môn học',
            'MaMH.array' => 'Dữ liệu môn học không hợp lệ',
            'MaMH.*.exists' => 'Một số môn học không tồn tại trong hệ thống'
        ]);

        if ($validator->fails()) {
            return redirect()->back()
                ->withErrors($validator)
                ->withInput();
        }

        try {
            foreach ($request->MaMH as $MaMH) {
                ChuongTrinhMonHoc::create([
                    'MaChuongTrinh' => $maChuongTrinh,
                    'MaMH' => $MaMH
                ]);
            }

            return redirect()->route('chuongtrinh.monhoc', $maChuongTrinh)
                ->with('success', 'Thêm các môn học vào chương trình thành công');
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Có lỗi xảy ra: ' . $e->getMessage());
        }
    }
    public function destroyMonHoc($maChuongTrinh, $MaMH)
    {
        try {
            ChuongTrinhMonHoc::where('MaChuongTrinh', $maChuongTrinh)
                ->where('MaMH', $MaMH)
                ->delete();

            return redirect()->route('chuongtrinh.monhoc', $maChuongTrinh)
                ->with('success', 'Xóa môn học khỏi chương trình thành công');
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Không thể xóa môn học: ' . $e->getMessage());
        }
    }
    public function xoaHinhThucDanhGia(Request $request)
    {
        $ids = $request->input('ids');
        $maChuongTrinh = $request->input('maChuongTrinh');

        try {
            HinhThucDanhGia::where('MaChuongTrinh', $maChuongTrinh)->whereIn('id', $ids)->delete();

            return redirect()->route('chuongtrinh.edit', $maChuongTrinh)
                ->with('success', 'Xóa môn học khỏi chương trình thành công');
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    }
}
