...
class MonHocController extends Controller
{
    ...
    {
        // $monhocs = MonHoc::get();
        $monhocs = MonHoc::with('giangViens')->get();

        // Kiểm tra dữ liệu
        foreach ($monhocs as $monhoc) {
            dd($monhoc->giangViens);
        }
    ...
    public function updateTeacherAssignment(Request $request, $MaMH, $maGV)
    {
        $validated = $request->validate([
            'MaGV' => 'required|exists:giaovien,MaGV',
            'MaLop' => 'required|array',
            'MaLop.*' => 'exists:lophoc,MaLop',
            'NgayBatDau' => 'nullable|date',
            'NgayKetThuc' => 'nullable|date|after:NgayBatDau',
        ]);

        $maGVNew = $validated['MaGV'];
        $newLops = $validated['MaLop'];
        $ngayBatDau = $validated['NgayBatDau'];
        $ngayKetThuc = $validated['NgayKetThuc'];
        $ghiChu = $request->input('GhiChu');

        // Lấy các lớp hiện đang được phân công cho giáo viên gốc
        $oldAssignments = \DB::table('giangday')
            ->where('MaMH', $MaMH)
            ->where('MaGV', $maGV)
            ->pluck('MaLop')
            ->toArray();

        // Các lớp cần xóa (không còn trong danh sách mới)
        $lopsToDelete = array_diff($oldAssignments, $newLops);
        // Các lớp cần thêm mới (chưa từng có)
        $lopsToInsert = array_diff($newLops, $oldAssignments);
        // Các lớp giữ nguyên → cần cập nhật nếu có thay đổi GV hoặc ngày
        $lopsToUpdate = array_intersect($oldAssignments, $newLops);

        // Xóa bản ghi không còn tồn tại trong danh sách mới
        if (!empty($lopsToDelete)) {
            \DB::table('giangday')
                ->where('MaMH', $MaMH)
                ->where('MaGV', $maGV)
                ->whereIn('MaLop', $lopsToDelete)
                ->delete();
        }

        // Cập nhật bản ghi lớp giữ nguyên nếu đổi thông tin
        foreach ($lopsToUpdate as $lop) {
            \DB::table('giangday')
                ->where('MaMH', $MaMH)
                ->where('MaGV', $maGV)
                ->where('MaLop', $lop)
        ...
        // Thêm bản ghi mới cho lớp chưa từng có
        foreach ($lopsToInsert as $lop) {
