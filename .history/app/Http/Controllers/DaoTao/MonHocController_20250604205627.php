...
class MonHocController extends Controller
{
    ...
    public function removeTeacher($tenMH, $maGV)
    {
        $monhoc = monhoc::findOrFail($tenMH);

        // Detach the specific teacher from this subject
        $monhoc->giangViens()->detach($maGV);

        return redirect()->route('monhoc.index')
            ->with('success', 'Đã huỷ phân công giảng viên khỏi môn học thành công');
    }
    public function editTeacherAssignment($tenMH, $maGV)
    {
        $monhoc = monhoc::findOrFail($tenMH);
        $giaoviens = giaovien::all();

        // Lấy thông tin giảng viên hiện tại
        $currentTeacher = $monhoc->giangViens()->where('MaGV', $maGV)->first();
    ...
    public function updateTeacherAssignment(Request $request, $tenMH, $maGV)
    {
        $request->validate([
            'MaGV' => 'required|exists:giaovien,MaGV',
            'NgayBatDau' => 'nullable|date',
        ...
        $monhoc->giangViens()->updateExistingPivot($maGV, [
            'MaGV' => $request->MaGV,
            'NgayBatDau' => $request->NgayBatDau,
            'NgayKetThuc' => $request->NgayKetThuc,
            'GhiChu' => $request->GhiChu
