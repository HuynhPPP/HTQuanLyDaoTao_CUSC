...
class LapBangDiemThiController extends Controller
{
    ...
    public function bangDiemTong($maChuongTrinh)
    {
        ...
        $danhSachSinhVien = DB::table('lophoc')
            ->join('danhsachsv', 'lophoc.MaLop', '=', 'danhsachsv.MaLop')
            ->where('lophoc.MaChuongTrinh', $maChuongTrinh)
            ->select('danhsachsv.MaSV')
            ->distinct()
            ->get();
    ...
    public function thongKeSinhVienDatMon($maChuongTrinh)
    {
        ...
        // Lấy danh sách môn học trong chương trình
        $danhSachMonHoc = DB::table('chuongtrinh_monhoc')
            ->join('monhoc', 'chuongtrinh_monhoc.TenMH', '=', 'monhoc.TenMH')
            ->where('MaChuongTrinh', $maChuongTrinh)
            ->select('monhoc.MaMH', 'monhoc.TenMH')
            ->get();

        // Thống kê số lượng sinh viên đạt/không đạt từng môn
        $thongKeDiemMon = $danhSachMonHoc->map(function ($monHoc) {
            // Lấy điểm thi của môn học
            $danhSachDiem = DB::table('diemthi')
                ->where('MaMH', $monHoc->MaMH)
                ->get();

            // Đếm số lượng sinh viên đạt và không đạt
            $sinhVienDat = $danhSachDiem->filter(function ($diemThi) {
                return $diemThi->DiemTong >= 40; // Điểm đạt là từ 40 trở lên
            })->count();

            $sinhVienKhongDat = $danhSachDiem->filter(function ($diemThi) {
                return $diemThi->DiemTong < 40; // Điểm không đạt là dưới 40
            })->count();

            return [
                'MaMH' => $monHoc->MaMH,
                'TenMH' => $monHoc->TenMH,
                'SinhVienDat' => $sinhVienDat,
                'SinhVienKhongDat' => $sinhVienKhongDat,
                'TongSinhVien' => $danhSachDiem->count()
            ];
        });

        // Tạo view thống kê
        return view('tochucthi.diemthi.thongke_sinhvien_datmon', [
            'chuongTrinh' => $chuongTrinh,
            'thongKeDiemMon' => $thongKeDiemMon
        ]);
    }
