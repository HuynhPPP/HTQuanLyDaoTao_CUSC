...
class LichThiController extends Controller
{
    ...
    private function validateLichThi(Request $request)
    {
        return $request->validate([
            ...
            'MaMH.string' => 'Tên môn học không hợp lệ.',
    ...
    public function store(Request $request)
    {
        ...
        $validated['MaLichThi'] = 'LT' . now()->format('ymdHis');
    ...
    public function update(Request $request, $id)
    {
        ...
        $gioKetThuc = Carbon::createFromFormat('H:i:s', $validated['GioBatDau'])
            ->addMinutes((int) $validated['ThoiLuong'])
            ->format('H:i');
    ...
    public function destroy($maLichThi)
    {
        try {
            $LichThi = LichThi::where('MaLichThi', $maLichThi)
                ->firstOrFail();
            
            $LichThi->delete();

            return redirect()->back()->with('success', 'Xoá lịch thi thành công');
        } catch (\Exception $e) {
