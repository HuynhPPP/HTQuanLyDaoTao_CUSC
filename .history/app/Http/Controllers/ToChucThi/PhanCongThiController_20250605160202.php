...
class PhanCongThiController extends Controller
{
    ...
    public function store(Request $request, $maLichThi)
    {
        ...
        try {
            // Kiểm tra số lượng cán bộ đã phân công
            $existingAssignments = PhieuPhanCongThi::where('MaLichThi', $maLichThi)->count();
            if ($existingAssignments + count($request->MaCB) > 10) {
                return redirect()->back()->with('error', 'Vượt quá số lượng cán bộ được phân công cho lịch thi.');
            }
