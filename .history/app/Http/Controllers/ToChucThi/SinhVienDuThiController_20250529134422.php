...
{
    // Hiển thị danh sách lịch thi để chọn
    public function chonLichThi()
    {
        $danhSachLichThi = LichThi::with(['lopHoc', 'monHoc'])
            ->orderBy('NgayThi', 'desc')
            ->get();

        return view('tochucthi.lichthi.chon_lich_thi', compact('danhSachLichThi'));
    }
    ...
    public function danhSachSinhVienDuThi($maLichThi)
    {
        ...
        $lichThi = LichThi::with(['lopHoc', 'monHoc'])->findOrFail($maLichThi);
    ...
    {
        $lichThi = LichThi::findOrFail($maLichThi);
        return Excel::download(new DanhSachSinhVienDuThiExport($maLichThi), "danh_sach_du_thi_{$lichThi->TenMH}.xlsx");
    }
}
