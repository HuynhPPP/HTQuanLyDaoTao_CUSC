<?php
namespace App\Http\Controllers\ToChucThi;

use App\Http\Controllers\Controller;
use App\Models\danhsachsv;
use App\Models\DiemThi;
use App\Models\LopHoc;
use App\Models\MonHoc;
use App\Imports\DiemThiImport;
use App\Exports\DiemThiExport;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Http\Request;

class BangDiemController extends Controller
{
    public function chonLopVaMon()
    {
        // Lấy danh sách lớp và môn học từ DB
        $dsLop = LopHoc::all();
        $dsMon = MonHoc::all();

        return view('tochucthi.diemthi.choose_schedules', compact('dsLop', 'dsMon'));
    }

    public function xemBangDiem(Request $request)
    {
        $maLop = $request->input('maLop');
        $tenMH = $request->input('tenMH');

        // Chuyển đến route có tên `bangdiem.show`
        return redirect()->route('bangdiem.show', ['maLop' => $maLop, 'tenMH' => $tenMH]);
    }


    public function show($maLop, $tenMH)
    {
        $lop = LopHoc::where('MaLop', $maLop)->first();
        $danhSachSV = DanhSachSV::with([
            'sinhVien',
            'diem' => function ($query) use ($tenMH) {
                $query->where('TenMH', $tenMH);
            }
        ])->where('MaLop', $maLop)->get();
        return view('tochucthi.diemthi.add_point', compact('maLop', 'tenMH', 'danhSachSV', 'lop'));
    }

    // public function import(Request $request)
    // {
    //     $maLop = $request->maLop;
    //     $tenMH = $request->tenMH;

    //     if ($request->hasFile('file')) {
    //         Excel::import(new DiemThiImport($maLop, $tenMH), $request->file('file'));
    //     } else {
    //         // Xử lý nhập điểm thủ công
    //         if ($request->has('diem') && is_array($request->diem)) {
    //             $successCount = 0;
    //             $errorMessages = [];

    //             foreach ($request->diem as $maSV => $diem) {
    //                 // Bỏ qua nếu điểm rỗng
    //                 if (empty($diem) && $diem !== '0') {
    //                     continue;
    //                 }

    //                 // Validate điểm
    //                 if (!is_numeric($diem) || $diem < 0 || $diem > 100) {
    //                     $errorMessages[] = "Điểm của sinh viên $maSV không hợp lệ (phải từ 0-100)";
    //                     continue;
    //                 }

    //                 $lanThi = $request->lanThi[$maSV] ?? 1;
    //                 $ghiChu = $request->ghiChu[$maSV] ?? null;

    //                 try {
    //                     // Sử dụng method createOrUpdateDiem để xử lý chính xác
    //                     DiemThi::createOrUpdateDiem($maSV, $tenMH, $maLop, $lanThi, $diem, $ghiChu);
    //                     $successCount++;
    //                 } catch (\Exception $e) {
    //                     $errorMessages[] = "Lỗi khi cập nhật điểm cho sinh viên $maSV: " . $e->getMessage();
    //                 }
    //             }

    //             // Xử lý trường hợp thay đổi lần thi (nếu có)
    //             if ($request->has('lanThi') && is_array($request->lanThi)) {
    //                 foreach ($request->lanThi as $maSV => $newLanThi) {
    //                     // Kiểm tra xem có thay đổi lần thi không
    //                     $existingRecord = DiemThi::where([
    //                         ['MaSV', '=', $maSV],
    //                         ['TenMH', '=', $tenMH]
    //                     ])->first();

    //                     if ($existingRecord && $existingRecord->LanThi != $newLanThi) {
    //                         // Nếu thay đổi lần thi, cần xóa record cũ và tạo record mới
    //                         // (vì LanThi là part of primary key)
    //                         try {
    //                             $oldData = [
    //                                 'MaLop' => $existingRecord->MaLop,
    //                                 'Diem' => $existingRecord->Diem,
    //                                 'GhiChu' => $existingRecord->GhiChu
    //                             ];

    //                             // Xóa record cũ
    //                             $existingRecord->delete();

    //                             // Tạo record mới với lần thi mới
    //                             DiemThi::create([
    //                                 'MaSV' => $maSV,
    //                                 'TenMH' => $tenMH,
    //                                 'MaLop' => $maLop,
    //                                 'LanThi' => $newLanThi,
    //                                 'Diem' => $oldData['Diem'],
    //                                 'GhiChu' => $oldData['GhiChu']
    //                             ]);
    //                         } catch (\Exception $e) {
    //                             $errorMessages[] = "Lỗi khi cập nhật lần thi cho sinh viên $maSV: " . $e->getMessage();
    //                         }
    //                     }
    //                 }
    //             }

    //             // Trả về thông báo phù hợp
    //             if ($successCount > 0) {
    //                 $message = "Đã cập nhật điểm thành công.";
    //                 if (!empty($errorMessages)) {
    //                     $message .= "\nCó một số lỗi:\n" . implode("\n", $errorMessages);
    //                     return redirect()->back()->with('warning', $message);
    //                 }
    //                 return redirect()->back()->with('success', $message);
    //             } else if (!empty($errorMessages)) {
    //                 return redirect()->back()->with('error', implode("\n", $errorMessages));
    //             }
    //         }
    //     }

    //     return redirect()->back()->with('info', 'Không có thay đổi nào được thực hiện.');
    // }

    public function import(Request $request)
    {
        $maLop = $request->maLop;
        $tenMH = $request->tenMH;

        if ($request->hasFile('file')) {
            Excel::import(new DiemThiImport($maLop, $tenMH), $request->file('file'));
        } else {
            // Xử lý nhập điểm thủ công
            if ($request->has('diem') && is_array($request->diem)) {
                $successCount = 0;
                $updateCount = 0; // Đếm số lần cập nhật lần thi
                $errorMessages = [];
                $processedStudents = []; // Theo dõi sinh viên đã xử lý

                // Xử lý điểm
                foreach ($request->diem as $maSV => $diem) {
                    // Bỏ qua nếu điểm rỗng
                    if (empty($diem) && $diem !== '0') {
                        continue;
                    }

                    // Validate điểm
                    if (!is_numeric($diem) || $diem < 0 || $diem > 100) {
                        $errorMessages[] = "Điểm của sinh viên $maSV không hợp lệ (phải từ 0-100)";
                        continue;
                    }

                    $lanThi = $request->lanThi[$maSV] ?? 1;
                    $ghiChu = $request->ghiChu[$maSV] ?? null;

                    try {
                        // Sử dụng updateOrCreate để xử lý chính xác
                        DiemThi::updateOrCreate(
                            [
                                'MaSV' => $maSV,
                                'TenMH' => $tenMH,
                                'LanThi' => $lanThi
                            ],
                            [
                                'MaLop' => $maLop,
                                'Diem' => (float) $diem,
                                'GhiChu' => $ghiChu
                            ]
                        );
                        $successCount++;
                        $processedStudents[] = $maSV;
                    } catch (\Exception $e) {
                        $errorMessages[] = "Lỗi khi cập nhật điểm cho sinh viên $maSV: " . $e->getMessage();
                    }
                }

                // Xử lý trường hợp chỉ thay đổi lần thi (không có điểm)
                if ($request->has('lanThi') && is_array($request->lanThi)) {
                    foreach ($request->lanThi as $maSV => $newLanThi) {
                        // Bỏ qua nếu sinh viên này đã được xử lý ở phần điểm
                        if (in_array($maSV, $processedStudents)) {
                            continue;
                        }

                        // Kiểm tra xem có record nào của sinh viên này không
                        $existingRecord = DiemThi::where([
                            ['MaSV', '=', $maSV],
                            ['TenMH', '=', $tenMH]
                        ])->first();

                        if ($existingRecord) {
                            // Nếu lần thi thay đổi
                            if ($existingRecord->LanThi != $newLanThi) {
                                try {
                                    // Lưu dữ liệu cũ
                                    $oldData = [
                                        'MaLop' => $existingRecord->MaLop,
                                        'Diem' => $existingRecord->Diem,
                                        'GhiChu' => $existingRecord->GhiChu
                                    ];

                                    // Xóa record cũ
                                    $existingRecord->delete();

                                    // Tạo record mới với lần thi mới
                                    DiemThi::create([
                                        'MaSV' => $maSV,
                                        'TenMH' => $tenMH,
                                        'MaLop' => $maLop,
                                        'LanThi' => $newLanThi,
                                        'Diem' => $oldData['Diem'],
                                        'GhiChu' => $oldData['GhiChu']
                                    ]);
                                    $updateCount++;
                                } catch (\Exception $e) {
                                    $errorMessages[] = "Lỗi khi cập nhật lần thi cho sinh viên $maSV: " . $e->getMessage();
                                }
                            }
                        } else {
                            // Nếu chưa có record nào, tạo mới với lần thi mới (nhưng không có điểm)
                            $ghiChu = $request->ghiChu[$maSV] ?? null;
                            try {
                                DiemThi::create([
                                    'MaSV' => $maSV,
                                    'TenMH' => $tenMH,
                                    'MaLop' => $maLop,
                                    'LanThi' => $newLanThi,
                                    'Diem' => null, // Chưa có điểm
                                    'GhiChu' => $ghiChu
                                ]);
                                $updateCount++;
                            } catch (\Exception $e) {
                                $errorMessages[] = "Lỗi khi tạo record cho sinh viên $maSV: " . $e->getMessage();
                            }
                        }
                    }
                }

                // Tạo thông báo chi tiết
                $messages = [];
                if ($successCount > 0) {
                    $messages[] = "Đã cập nhật điểm cho $successCount sinh viên";
                }
                if ($updateCount > 0) {
                    $messages[] = "Đã cập nhật lần thi cho $updateCount sinh viên";
                }

                // Trả về thông báo phù hợp
                if (!empty($messages)) {
                    $message = implode(" và ", $messages) . " thành công.";
                    if (!empty($errorMessages)) {
                        $message .= "\nCó một số lỗi:\n" . implode("\n", $errorMessages);
                        return redirect()->back()->with('warning', $message);
                    }
                    return redirect()->back()->with('success', $message);
                } else if (!empty($errorMessages)) {
                    return redirect()->back()->with('error', implode("\n", $errorMessages));
                }
            }
        }

        return redirect()->back()->with('info', 'Không có thay đổi nào được thực hiện.');
    }
    public function export($maLop, $tenMH)
    {
        return Excel::download(new DiemThiExport($maLop, $tenMH), "diem-thi-{$maLop}-{$tenMH}.xlsx");
    }
}