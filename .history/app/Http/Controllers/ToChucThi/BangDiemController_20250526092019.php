        ...
        if ($request->hasFile('file')) {
            ...
            if ($request->has('diem') && is_array($request->diem)) {
                ...
                foreach ($request->diem as $maSV => $diem) {
                    ...
                    try {
                        // Tìm tất cả record của sinh viên cho môn học này
                        ...
                        foreach ($existingRecords as $record) {
                            if ($record->LanThi == $lanThi) {
                                // Cập nhật record hiện có nếu lần thi trùng khớp
                                $record->update([
                                    'MaLop' => $maLop,
                                    'Diem' => (float) $diem,
                                    'GhiChu' => $ghiChu
                        ...
                        if (!$recordExists) {
                            // Tạo record mới nếu không tìm thấy record nào
                            DiemThi::create([
                                'MaSV' => $maSV,
                                'TenMH' => $tenMH,
                                'MaLop' => $maLop,
                ...
                if ($successCount > 0) {
                    $message = "Đã cập nhật thành công $successCount điểm.";
