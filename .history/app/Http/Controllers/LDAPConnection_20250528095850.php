...
class LDAPConnection extends Controller
{
    ...
    public function getAllADAccounts()
    {
        ...
        try {
            ...
            // Bind với tài khoản admin
            $bind = ldap_bind($ds, 'CUSC\\' . config('ldap.admin_username'), config('ldap.admin_password'));
            if (!$bind) {
                throw new Exception('Không thể xác thực với máy chủ LDAP.');
            }

            // Tìm kiếm tất cả user
            $search = ldap_search($ds, $ldapconfig['basedn'], '(&(objectClass=user)(objectCategory=person))');
            if (!$search) {
