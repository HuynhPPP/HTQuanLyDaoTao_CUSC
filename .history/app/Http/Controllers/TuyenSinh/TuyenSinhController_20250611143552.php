...
class TuyenSinhController extends Controller
{
    ...
    public function danhSachHoSo($maTS)
    {
        ...
        $hoSos = HoSoTuyenSinh::with(['sinhvien', 'thongtintuyensinh'])
            ->where('MaTS', $maTS)
            ->orderBy('NgayNopHS', 'desc')
            ->get();

        return view('quanly_tuyensinh.tuyensinh.danhsach_hoso', compact('hoSos', 'dotTuyenSinh'));
    }
    ...
    public function taoHoSo(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'MaTS' => 'required|exists:ThongTinTuyenSinh,MaTS',
            ...
            'MaTS.exists' => 'Đợt tuyển sinh không tồn tại trong hệ thống',
    ...
    public function capNhatHoSo(Request $request, $maHoSo)
    {
        $validator = Validator::make($request->all(), [
            'Hinh3X4' => 'nullable|boolean',
            'HinhCCCD' => 'nullable|boolean',
            'ToDangKi' => 'nullable|boolean',
        ...
        if ($validator->fails()) {
            return redirect()->back()
                ->withErrors($validator)
                ->withInput()
                ->with('error', 'Cập nhật hồ sơ không thành công');
        }
    
        $hoSo = HoSoTuyenSinh::findOrFail($maHoSo);
        
        $hoSo->update([
            'Hinh3X4' => $request->input('Hinh3X4', $hoSo->Hinh3X4),
            'HinhCCCD' => $request->input('HinhCCCD', $hoSo->HinhCCCD),
            'ToDangKi' => $request->input('ToDangKi', $hoSo->ToDangKi),
