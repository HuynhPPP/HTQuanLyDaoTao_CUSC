...
class AddPointExamController extends Controller
{
    ...
    public function nhapDiem($MaLop, $MaMH)
    {
        ...
        $monHoc = monhoc::where('MaMH', $MaMH)->first();

        // Tìm mối liên kết môn học với chương trình
        $chuongTrinhMonHoc = ChuongTrinhMonHoc::where('MaMH', $monHoc->MaMH)->first();

        if (!$chuongTrinhMonHoc) {
            return back()->with('error', 'Không tìm thấy chương trình đào tạo của môn học');
        }

        $maChuongTrinh = $chuongTrinhMonHoc->MaChuongTrinh;

        // Lấy danh sách hình thức đánh giá của chương trình này
        $hinhThucDanhGia = HinhThucDanhGia::where('MaChuongTrinh', $maChuongTrinh)
            ->pluck('TiLePhanTram', 'HinhThuc') // ['Lý thuyết trắc nghiệm' => 50, ...]
            ->toArray();
    ...
    // public function luuDiem(Request $request, $MaLopHoc, $MaMH)
    // {
    //     // dd([
    //     //     'request' => $request->all(),
    //     //     'MaLopHoc' => $MaLopHoc,
    ...
    //         return redirect()->route('giaovien.nhapdiemthi.nhap-diem', [
    //             'MaLop' => $MaLopHoc,
    //             'MaMH' => $MaMH
    //         ])
    ...
    public function luuDiem(Request $request, $MaLopHoc, $MaMH)
{
    ...
    $lop = DB::table('lophoc')->where('MaLop', $MaLopHoc)->first();
    if (!$lop) {
        return redirect()->back()->with('error', 'Không tìm thấy lớp học');
    }

    $maChuongTrinh = $lop->MaChuongTrinh;

    $hinhThucMap = DB::table('hinhthucdanhgia')
        ->where('MaChuongTrinh', $maChuongTrinh)
        ->pluck('TiLePhanTram', 'HinhThuc')
        ->toArray();

    // ✅ Kiểm tra tổng tỷ lệ
    $tongTiLe = array_sum($hinhThucMap);
    if ($tongTiLe !== 100) {
        return redirect()->back()->with('error', 'Tổng tỷ lệ các hình thức đánh giá phải bằng 100%. Hiện tại: ' . $tongTiLe . '%');
    }
    ...
    try {
        foreach ($request->diems as $maSV => $diem) {
            ...
            $diemLyThuyet = floatval($diem['DiemLyThuyet'] ?? 0);
            $diemThucHanh = floatval($diem['DiemThucHanh'] ?? 0);
            $diemDuAn = floatval($diem['DiemBaiTap'] ?? 0);

            $diemTong = (
                $diemLyThuyet * ($hinhThucMap['Lý thuyết trắc nghiệm'] ?? 0) +
                $diemThucHanh * ($hinhThucMap['Thực hành'] ?? 0) +
                $diemDuAn * ($hinhThucMap['Dự án'] ?? 0)
            ) / 100;

            $diemThi->DiemLyThuyet = $diemLyThuyet;
            $diemThi->DiemThucHanh = $diemThucHanh;
            $diemThi->DiemDuAn = $diemDuAn;
            $diemThi->DiemTong = round($diemTong, 2);
            $diemThi->GhiChu = $diem['GhiChu'] ?? null;
