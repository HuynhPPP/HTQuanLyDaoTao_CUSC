                    ...
                    if ($element instanceof \PhpOffice\PhpWord\Element\TextRun) {
                        foreach ($element->getElements() as $textElement) {
                            if ($textElement instanceof \PhpOffice\PhpWord\Element\Text) {
                                ...
                                // Trích xuất Lớp, Ngày, Giờ, Địa điểm như cũ
                        ...
                        foreach ($element->getRows() as $row) {
                            foreach ($row->getCells() as $cell) {
                                ...
                                // Tìm trong bảng "THÀNH PHẦN"
                                if (str_contains($cellText, 'Giáo viên hướng dẫn')) {
                                    // Giả định tên giáo viên nằm trong ô kế tiếp hoặc cùng hàng
                                    // Cần kiểm tra cấu trúc cụ thể của bảng trong file Word của bạn
                                    // Ví dụ: nếu tên nằm trong cột thứ 2 của hàng chứa "Giáo viên hướng dẫn"
                                    $instructor_name = trim($row->getCells()[1]->getText()); // Cần điều chỉnh index tùy thuộc vào cấu trúc bảng
                                }
                                if (str_contains($cellText, 'Giáo viên phản biện')) {
                                    $reviewer_name = trim($row->getCells()[1]->getText()); // Cần điều chỉnh index
                                }
                                // Hoặc nếu tên nằm trong cột thứ 2 của bảng "LỊCH BÁO CÁO"
                                if (str_contains($cellText, 'GIÁO VIÊN HƯỚNG DẪN')) {
                                    // Lấy tên từ hàng tiếp theo hoặc ô cụ thể
                                    // Ví dụ: $row->getCells()[2]->getText() nếu tên nằm ở cột thứ 3
                                    // Đây là phần cần bạn điều chỉnh dựa trên file Word thật
                                }
                            }
                        }
                    }
