...
class ThongKeDashboardController extends Controller
{
    ...
    public function dashboard($MaLop, $MaChuongTrinh)
{
    ...
    $tongKet = $dsDiem->groupBy('MaSV')->map(function ($ds) use ($tieuChi) {
        ...
        $loai = 'Chưa xếp loại';
        foreach ($tieuChi as $tc) {
            if ($tb >= $tc->DiemTu && $tb < $tc->DiemDen) $loai = $tc->XepLoai;
        }
        return [
            'MaSV' => $ds->first()->MaSV,
    ...
    return view('thongke.dashboard', compact('lop', 'theoMon', 'thongKeDat', 'tongKet'));
}
