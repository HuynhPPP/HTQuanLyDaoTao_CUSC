...
class SinhVienController extends Controller
{
    ...
    public function dongBoTaiKhoanLDAP()
    {
        ...
        try {
            // Kết nối LDAP
            // Kết nối LDAP sử dụng LDAPS
$ds = ldap_connect("ldaps://{$ldapconfig['host']}", 636); // ⚠️ LDAPS, port 636
if (!$ds) {
    throw new \Exception('Không thể kết nối đến máy chủ LDAP (LDAPS)');
}
            ...
            foreach ($sinhViens as $sinhVien) {
                ...
                try {
                    ...
                    // $plainPassword = Str::random(8); // Tạo mật khẩu ngẫu nhiên
                    // $hashedPassword = Hash::make($plainPassword);
                    // $encodedPassword = iconv('UTF-8', 'UTF-16LE', '"' . $plainPassword . '"');
                    // $passwordSet = ldap_mod_replace($ds, $dn, ['unicodePwd' => $encodedPassword]);
                    ...
                    // // Kích hoạt tài khoản
                    // $enableAccount = ldap_mod_replace($ds, $dn, ['userAccountControl' => 512]);
