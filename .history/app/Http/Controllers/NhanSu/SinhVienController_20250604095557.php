...
class SinhVienController extends Controller
{
    ...
    public function changePassword(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'current_password' => [
                'required', 
                function($attribute, $value, $fail) use ($sinhVien) {
                    // Kiểm tra mật khẩu hiện tại
                    $ldapAccount = LdapAccount::where('username', $sinhVien->EmailCUSC)->first();
                    
                    if (!$ldapAccount || !Hash::check($value, $ldapAccount->password)) {
        ...
        try {
            ...
            if (!$ldapAccount) {
                return response()->json([
                    'success' => false, 
                    'message' => 'Không tìm thấy tài khoản LDAP'
                ], 404);
            }

            // Mã hóa mật khẩu mới
            $ldapAccount->password = Hash::make($request->new_password);
            $ldapAccount->save();
            ...
            Log::error('Lỗi đổi mật khẩu sinh viên: ' . $e->getMessage());
            return response()->json([
                'success' => false,
