...
class CanBoController extends Controller
{
            ...
            foreach ($canBos as $canBo) {
                ...
                try {
                    ...
                    if (!$addToGroupResult) {
                        $error = ldap_error($ds);
                        $errno = ldap_errno($ds);
                        Log::warning("Không thể thêm user vào group Staff: $error (Mã lỗi: $errno)");
                    }
    ...
    public function xuatDanhSachTaiKhoanMoi()
    {
        ...
        return view('quanly_nhansu.canbo.list_account', compact('ldapAccounts'));
    }
    ...
    private function taoEmailCUSC($canBo)
    {
        // Bỏ dấu và ký tự đặc biệt, chuyển thành chữ thường
        $hoTenKhongDau = $this->loaiBoKyTuDacBiet($canBo->HoTenCB);
        $tenKhongDau = strtolower(preg_replace('/\s+/', '', $hoTenKhongDau));

        // Tạo email với MaGV ở đầu
        $baseEmail = strtolower($canBo->MaCB . $tenKhongDau);
    ...
    public function kiemTraDongBoLDAP()
    {
        ...
        try {
            ...
            $giaoViensCount = canbo::whereNull('EmailCUSC')
                // ->orWhereNull('password_CUSC')
                ->count();
            ...
            return response()->json([
                ...
                'so_luong_can_bo_chua_co_tai_khoan' => $giaoViensCount
            ]);
    ...
    public function changePassword(Request $request)
    {
        ...
        $id = session('id');
        $ldapAccount = LdapAccount::where('role', 'admin')->where('MaTaiKhoan', $id)->first();
        ...
        $validator = Validator::make($request->all(), [
            'current_password' => [
                ...
                function ($attribute, $value, $fail) use ($ldapAccount) {
                    ...
                    if (!Hash::check($value, $ldapAccount->initial_password)) {
        ...
        try {
            ...
            \Log::error('Lỗi đổi mật khẩu admin: ' . $e->getMessage());
            return response()->json([
                'success' => false,
