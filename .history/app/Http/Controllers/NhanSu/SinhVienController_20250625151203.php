...
class SinhVienController extends Controller
{
    ...
    public function kiemTraDongBoLDAP()
    {
        ...
        try {
            ...
            if (!$sessionUser || !in_array($sessionRole, ['admin'])) {
                throw new \Exception('Chỉ có tài khoản quản trị có quyền thực hiện thao tác này.');
            }
            ...
            // Sử dụng tên người dùng từ session
            $adminUsername = $sessionUser;

            // Lấy mật khẩu từ request hoặc yêu cầu nhập lại
            $adminPassword = request()->input('password');

            // Nếu không có mật khẩu, yêu cầu nhập lại
            ...
            if (!@ldap_bind($ds, $bindString, $adminPassword)) {
                throw new \Exception('Kết nối LDAP thất bại. Kiểm tra lại mật khẩu.');
            }
