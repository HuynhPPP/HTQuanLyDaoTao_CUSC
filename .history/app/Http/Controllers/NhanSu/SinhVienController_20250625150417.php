...
class SinhVienController extends Controller
{
    ...
    {
        // Lấy cấu hình LDAP từ biến môi trường
        ...
        // Lấy thông tin đăng nhập quản trị từ biến môi trường
        ...
        try {
            ...
            // Thử bind với tài khoản quản trị
            $bind_string = $domain . '\\' . $admin_username;

            $bind = ldap_bind($ds, $bind_string, $admin_password);
            if (!$bind) {
                $error = ldap_error($ds);
                $errno = ldap_errno($ds);
                throw new \Exception("Kết nối LDAP thất bại: $error (Mã lỗi: $errno)");
