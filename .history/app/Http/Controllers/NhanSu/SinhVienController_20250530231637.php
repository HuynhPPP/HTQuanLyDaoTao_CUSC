        ...
        try {
            ...
            foreach ($sinhViens as $sinhVien) {
                try {
                    ...
                    // Tạo DN cho user mới
                    $dn = "CN={$sinhVien->HoTen},CN=Users,{$ldapconfig['basedn']}";

                    // Thêm user vào LDAP
                    $addResult = ldap_add($ds, $dn, $entry);
                    if (!$addResult) {
                        $error = ldap_error($ds);
                        throw new \Exception("Không thể tạo user LDAP: $error");
                    }
                    ...
                    if (!$passwordModify) {
                        $error = ldap_error($ds);
                        throw new \Exception("Không thể đặt mật khẩu: $error");
                    }
                    ...
                    if (!$addToGroupResult) {
                        $error = ldap_error($ds);
                        Log::warning("Không thể thêm user vào group Student: $error");
                    }
