        ...
        try {
            ...
            foreach ($sinhViens as $sinhVien) {
                try {
                    ...
                    // Tạo DN với định dạng phù hợp
                    $dn = "CN={$sanitizedEmail},OU=Student,{$ldapconfig['basedn']}";

                    // Bổ sung thêm các thuộc tính để tăng khả năng tạo user
                    $entry = [
                    ...
                    if (!$addResult) {
                        ...
                        switch ($errno) {
                            case 50:  // Insufficient access
                                throw new \Exception("Lỗi quyền truy cập: Tài khoản không có đủ quyền để tạo user LDAP");
                            case 68:  // Entry Already Exists
                                throw new \Exception("User đã tồn tại trong hệ thống LDAP");
                            default:
                                throw new \Exception("Lỗi không xác định khi thêm user LDAP: $error (Mã lỗi: $errno)");
                        }
                    }

                    // Thay đổi cách đặt mật khẩu
                    try {
                        // Mã hóa mật khẩu theo chuẩn UTF-16LE
                        $newPassword = '"' . $password . '"';
                        $encodedPassword = iconv('UTF-8', 'UTF-16LE', $newPassword);

                        // Thử đặt mật khẩu với các tùy chọn bổ sung
                        $modifications = [
                            'unicodePwd' => $encodedPassword,
                            'userAccountControl' => 512 // Kích hoạt tài khoản, vô hiệu hóa tài khoản bị khóa
                        ];

                        // Thực hiện thay đổi
                        $passwordModify = ldap_mod_replace($ds, $dn, $modifications);
                        ...
                        if (!$passwordModify) {
                            ...
                            // Log chi tiết lỗi
                            Log::error('Chi tiết lỗi đặt mật khẩu LDAP', [
                                'error_message' => $error,
                                'error_number' => $errno,
                                'dn' => $dn,
                        ...
                        Log::error('Ngoại lệ khi đặt mật khẩu LDAP', [
