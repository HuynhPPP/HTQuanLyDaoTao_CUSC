...
class SinhVienController extends Controller
{
    ...
    public function dongBoTaiKhoanLDAP()
    {
        ...
        try {
            // Kết nối LDAP với thông số chi tiết
            ...
            Log::debug('Số sinh viên cần đồng bộ: ' . $sinhViens->count());
            ...
            foreach ($sinhViens as $sinhVien) {
                try {
                    ...
                    Log::debug('Xử lý sinh viên: ' . $sinhVien->MaSV, [
                        'email' => $fullEmail,
                        'password' => $password
                    ]);
                    ...
                    if (!$saveResult) {
                        throw new \Exception('Không thể lưu user vào LDAP');
                    }

                    // Đặt mật khẩu
                    $ldapUser->setPassword($password);

                    // Thêm vào nhóm Student
                    ...
                    $successCount++;
                    Log::debug('Đồng bộ thành công cho sinh viên: ' . $sinhVien->MaSV);

                } catch (\Exception $userException) {
                    $errorCount++;
                    Log::error('Lỗi đồng bộ cho sinh viên ' . $sinhVien->MaSV . ': ' . $userException->getMessage());
                    continue;
                }
            }
            ...
            Log::error('Lỗi đồng bộ LDAP toàn bộ: ' . $e->getMessage());
    ...
    public function kiemTraDongBoLDAP()
    {
        ...
        try {
            $connection = new Connection([
                ...
                'username' => 'admin.khuong@cusc.ctu.vn',
                'password' => 'P@ssW@rd2025!',
                'use_ssl' => false,
                'use_tls' => false,
            ]);
