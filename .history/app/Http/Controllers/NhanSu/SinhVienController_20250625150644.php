...
class SinhVienController extends Controller
{
    ...
    public function kiemTraDongBoLDAP()
    {
        ...
        try {
            // Cấu hình LDAP từ biến môi trường
            $ldapConfig = [
                'host' => env('LDAP_HOST', '10.0.0.2'),
                'port' => env('LDAP_PORT', 389),
                'basedn' => env('LDAP_BASE_DN', 'dc=cusc,dc=ctu,dc=vn'),
                'domain' => env('LDAP_DOMAIN', 'CUSC')
            ];

            // Lấy thông tin đăng nhập quản trị
            $adminUsername = env('LDAP_ADMIN_USERNAME');
            $adminPassword = env('LDAP_ADMIN_PASSWORD');

            // Kiểm tra tính hợp lệ của thông tin đăng nhập
            if (empty($adminUsername) || empty($adminPassword)) {
                Log::error('LDAP Admin Credentials Missing');
                return response()->json([
                    'status' => 'error',
                    'message' => 'Thiếu thông tin đăng nhập quản trị LDAP'
                ], 500);
            }

            // Chuẩn bị bind string
            $bindString = $ldapConfig['domain'] . '\\' . $adminUsername;
            ...
            // Thử bind
            if (!@ldap_bind($ds, $bindString, $adminPassword)) {
                throw new \Exception('Kết nối LDAP thất bại. Kiểm tra lại thông tin đăng nhập.');
            }

            // Tìm kiếm thông tin người dùng (nếu cần)
            $search = ldap_search($ds, $ldapConfig['basedn'], "(sAMAccountName=$adminUsername)");
            ...
            return response()->json([
                ...
                'ldap_user_info' => $entries['count'] > 0 ? [
                    'display_name' => $entries[0]['displayname'][0] ?? $adminUsername,
                    'groups' => array_filter($entries[0]['memberof'] ?? [], function($key) {
                        return $key !== 'count';
