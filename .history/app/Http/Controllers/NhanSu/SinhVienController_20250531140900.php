...
class SinhVienController extends Controller
{
    ...
    public function dongBoTaiKhoanLDAP()
    {
        ...
        try {
            ...
            // Kiểm tra và tạo OU nếu chưa tồn tại
            $this->kiemTraVaTaoOU($ds, $ldapconfig['basedn']);
            ...
            foreach ($sinhViens as $sinhVien) {
                ...
                try {
                    ...
                    // Tạo DN cho user mới
                    $dn = "CN={$sanitizedEmail},OU=Students,{$ldapconfig['basedn']}";

                    // Thông tin entry LDAP
                    $entry = [
                    ...
                    if (!$addResult) {
                        ...
                        switch ($errno) {
                            ...
                            case 32:  // No Such Object
                                throw new \Exception("Không tìm thấy đối tượng LDAP: Kiểm tra lại OU và DN");
                            default:
    ...
    // Phương thức kiểm tra và tạo OU
    private function kiemTraVaTaoOU($ds, $basedn)
    {
        // Danh sách các OU cần kiểm tra và tạo
        $requiredOUs = [
            "OU=Students,{$basedn}"
        ];

        foreach ($requiredOUs as $ou) {
            // Tách các phần của DN
            $parts = explode(',', $ou);
            $currentDN = $ou;

            // Kiểm tra từng phần của DN
            foreach ($parts as $part) {
                try {
                    // Tìm kiếm DN
                    $search = ldap_search($ds, $currentDN, '(objectClass=*)');
                    $entries = ldap_get_entries($ds, $search);

                    // Nếu không tìm thấy, tạo mới
                    if ($entries['count'] == 0) {
                        // Xác định loại đối tượng (OU hay CN)
                        $type = strpos($part, 'OU=') === 0 ? 'organizationalUnit' : 'container';

                        // Thông tin entry mới
                        $entry = [
                            'objectClass' => ['top', $type],
                            'ou' => [substr($part, 3)],
                            'cn' => [substr($part, 3)]
                        ];

                        // Thêm entry mới
                        $addResult = ldap_add($ds, $currentDN, $entry);
                        if (!$addResult) {
                            $error = ldap_error($ds);
                            throw new \Exception("Không thể tạo OU: $currentDN. Lỗi: $error");
                        }

                        Log::info("Đã tạo OU: $currentDN");
                    }
                } catch (\Exception $e) {
                    Log::error('Lỗi kiểm tra/tạo OU', [
                        'dn' => $currentDN,
                        'error' => $e->getMessage()
                    ]);
                    throw $e;
                ...
                $currentDN = implode(',', array_slice(explode(',', $currentDN), 1));
            }
        }
    }
