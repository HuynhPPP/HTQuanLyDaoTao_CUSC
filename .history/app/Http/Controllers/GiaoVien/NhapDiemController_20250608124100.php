...
class NhapDiemController extends Controller
{
    ...
    public function luuDiem(Request $request, $maLichThi)
    {
        ...
        // Validate dữ liệu dựa trên hình thức thi
        $validationRules = [
            'diems' => 'required|array',
            'diems.*.LanThi' => 'required|numeric|min:1|max:3'
        ];

        // Thêm validation cho từng hình thức thi
        switch ($lichThi->HinhThucThi) {
            case 'Trắc nghiệm':
                $validationRules['diems.*.DiemLyThuyet'] = 'nullable|numeric|min:0|max:10';
                break;
            case 'Thực hành':
                $validationRules['diems.*.DiemThucHanh'] = 'nullable|numeric|min:0|max:10';
                break;
            case 'Dự án':
                $validationRules['diems.*.DiemDuAn'] = 'nullable|numeric|min:0|max:10';
                break;
            default:
                $validationRules['diems.*.Diem'] = 'nullable|numeric|min:0|max:10';
        }
        ...
        try {
            foreach ($request->diems as $maSV => $diem) {
                ...
                // Cập nhật điểm dựa trên hình thức thi
                switch ($lichThi->HinhThucThi) {
                    case 'Trắc nghiệm':
                    ...
                    case 'Thực hành':
                        ...
                        break;
                    case 'Dự án':
                        $diemThi->DiemDuAn = $diem['DiemDuAn'] ?? null;
                        break;
                    default:
