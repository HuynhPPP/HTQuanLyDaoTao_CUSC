...
class NhapDiemController extends Controller
{
    ...
    public function luuDiem(Request $request, $MaLopHoc, $MaMH)
    {
        ...
        if (!$MaLopHoc) {
            \Log::error('No MaLopHoc parameter provided');
        ...
        // Lấy thông tin lớp học
        $lop = LopHoc::where('MaLop', $MaLopHoc)->first();
        if (!$lop) {
            \Log::error('Lop not found', ['MaLop' => $MaLopHoc]);
            return redirect()->back()->with('error', 'Không tìm thấy lớp học: ' . $MaLopHoc);
        }
        ...
        try {
            ...
            return redirect()->route('giaovien.nhapdiemthi.nhap-diem', ['MaLop' => $MaLopHoc])
