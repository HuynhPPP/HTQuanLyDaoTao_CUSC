...
class NhapDiemController extends Controller
{
    ...
    public function luuDiem(Request $request, $lophoc)
    {
        ...
        if (!$lophoc) {
            \Log::error('No lophoc parameter provided');
        ...
        // Lấy thông tin lớp học
        $lop = LopHoc::where('MaLop', $lophoc)->first();
        if (!$lop) {
            \Log::error('Lop not found', ['lophoc' => $lophoc]);
            return redirect()->back()->with('error', 'Không tìm thấy lớp học: ' . $lophoc);
        }
        ...
        try {
            ...
            return redirect()->route('giaovien.nhapdiemthi.nhap-diem', ['lophoc' => $lop->MaLop])
