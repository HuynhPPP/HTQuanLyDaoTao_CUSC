    ...
    public function import(Request $request)
{
    ...
    // Giả sử dữ liệu ở sheet đầu tiên, dòng đầu là tiêu đề
    $rows = $data[0];
    ...
    foreach ($rows as $index => $row) {
        ...
        $rowData['GioiTinh'] = ($rowData['GioiTinh'] ?? '') === 'Nam' ? 1 : 0;
        ...
        try {
            \App\Models\sinhvien::create($rowData);
            $success++;
        } catch (\Exception $e) {
            $fail++;
    ...
    $msg = "Import thành công $success dòng. Thất bại $fail dòng.";
    if ($fail > 0) $msg .= '<br>' . implode('<br>', $errors);
