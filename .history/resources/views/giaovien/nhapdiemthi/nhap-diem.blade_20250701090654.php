            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form
                        ...
                        @csrf
                        @foreach ($hinhThucDanhGia as $ten => $tiLe)
                            <input type="hidden" id="tile-{{ \Str::slug($ten) }}" value="{{ $tiLe }}">
                        @endforeach
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Tỷ lệ từ hidden inputs
        const tileLyThuyet = parseFloat(document.getElementById('tile-ly-thuyet-trac-nghiem')?.value || 0);
        const tileThucHanh = parseFloat(document.getElementById('tile-thuc-hanh')?.value || 0);
        const tileDuAn = parseFloat(document.getElementById('tile-du-an')?.value || 0);

        table.querySelectorAll('input').forEach(input => {
            input.addEventListener('input', function () {
                const row = input.closest('tr');
                const thucHanh = parseFloat(row.querySelector('[name*="[DiemThucHanh]"]')?.value) || 0;
                const lyThuyet = parseFloat(row.querySelector('[name*="[DiemLyThuyet]"]')?.value) || 0;
                const baiTap = parseFloat(row.querySelector('[name*="[DiemBaiTap]"]')?.value) || 0;

                const diemTong = (
                    lyThuyet * tileLyThuyet +
                    thucHanh * tileThucHanh +
                    baiTap * tileDuAn
                ) / 100;
