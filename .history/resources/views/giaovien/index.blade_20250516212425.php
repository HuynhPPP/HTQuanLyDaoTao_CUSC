                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped" id="table-giaovien">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach($giaoviens as $index => $gv)
                                            <tr>
                                                <td class="text-center">{{ $giaoviens->firstItem() + $index }}</td>
                                                <td>{{ $gv->MaGV }}</td>
                                                <td>{{ $gv->HoTenGV }}</td>
                                                <td>{{ $gv->Email }}</td>
                                                <td>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        Hiển thị {{ $giaoviens->firstItem() }} - {{ $giaoviens->lastItem() }} 
                                        trong tổng số {{ $giaoviens->total() }} giáo viên
                                    </div>
                                    <div>
                                        {{ $giaoviens->links() }}
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#table-giaovien').DataTable({
                ...
                columnDefs: [
                    { 
                        targets: -1, // Cột cuối cùng (Hành Động)
                        sortable: false 
                    }
                ]
            });
