...
@section('main-content')
    ...
    @section('custom-js')
        ...
        <script>
            $(document).ready(function() {
                // Kích hoạt modal khi cần (giả sử sử dụng Bootstrap Modal)
                $('#modal-addTime').modal({
                    show: false // Không hiển thị ngay khi tải trang
                });
        
                // Xử lý sự kiện submit form (tùy chọn)
                $('#absenceForm').on('submit', function(e) {
                    e.preventDefault(); // Ngăn submit mặc định để kiểm tra
        
                    // Kiểm tra dữ liệu (ví dụ)
                    let tenNgayNghi = $('#TenNgayNghi').val().trim();
                    let ngayBDNghi = $('#NgayBDNghi').val();
                    let ngayKT = $('#NgayKT').val();
        
                    if (!tenNgayNghi || !ngayBDNghi || !ngayKT) {
