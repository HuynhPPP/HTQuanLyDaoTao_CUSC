                                ...
                                <form action="{{ route('student.store') }}" method="POST">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="MaSV" class="form-label">Mã sinh viên <span
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="NgaySinh" class="form-label">Ngày sinh <span
                                                    class="text-danger">*</span></label>
                                            <input type="date"
                                                class="form-control @error('NgaySinh') is-invalid @enderror" id="NgaySinh"
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="SoCCCD" class="form-label">Số CCCD <span
                                                    class="text-danger">*</span></label>
                                            <input type="text"
                                                class="form-control @error('SoCCCD') is-invalid @enderror" id="SoCCCD"
                                                name="SoCCCD" value="{{ old('SoCCCD') }}">
                                            @error('SoCCCD')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <h6 class="font-weight-bold">Lưu ý khi chuẩn bị file Excel:</h6>
                        <ol>
                            <li><strong>Cần giữ nguyên thứ tự 34 cột</strong> như mẫu. Không được đổi tên hay thêm bớt cột.
                            </li>
                            <li><strong>Các cột bắt buộc:</strong> <code>MaSV, HoTen, NgaySinh, GioiTinh, SoCCCD, Email,
                                    Sdt, TinhTrangHocTap</code></li>
                            <li><code>GioiTinh</code> chỉ chấp nhận <code>Nam</code> hoặc <code>Nữ</code></li>
                            <li><code>TinhTrangHocTap</code> nên nhập: <code>DangHoc</code>, <code>TotNghiep</code>,
                                <code>ThoiHoc</code>…</li>
                            <li><code>NgaySinh</code>, <code>NgayCap</code>, <code>NgayDangKi</code> theo định dạng
                                <code>yyyy-mm-dd</code></li>
                        </ol>
                    </div>

                    <h6 class="font-weight-bold">Danh sách 34 cột theo thứ tự:</h6>
                    <div class="mb-3">
                        <code class="d-block bg-light p-2 rounded" style="white-space: pre-wrap; word-break: break-word;">
                            MaSV, MaEnroll, HoTen, InDebt, NgaySinh, GioiTinh, SoCCCD, NgayCap, NoiCap, Sdt, NoiSinh,
                            DiaChi, Zalo, Receipt, Invoice, Billing, Coll, Billing(VND), Coll(VND), Discount, LiDo,
                            NgayDangKi, HoTenNguoiThan, MoiQuanHe, SdtNguoiThan, ZaloNguoiThan, EmailNguoiThan, Email,
                            EmailCUSC, password_CUSC, Size, TinhTrangHocTap, created_at, updated_at
                        </code>
                    </div>

                    <h6 class="font-weight-bold">Ví dụ dữ liệu mẫu:</h6>
                        ...
                        <table class="table table-bordered table-sm table-striped mb-3">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>Email</th>
                                    <th>Sdt</th>
                                    <th>TinhTrangHocTap</th>
                                    <th>HoTenNguoiThan</th>
                                    <th>SdtNguoiThan</th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <div class="alert alert-warning">
                        ...
                        Nếu bạn không chắc chắn, hãy sử dụng nút <strong>"Tải file mẫu"</strong> hoặc <strong>"Sao chép dữ
                            liệu mẫu"</strong> để tránh lỗi định dạng!
                    </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        <ol>
                            <li>Các cột bắt buộc: <code>masv, hoten, ngaysinh, gioitinh, socccd, email, sdt, diachi</code>
                            </li>
...
@section('custom-js')
    ...
    <script>
        ...
        function copyTemplateToClipboard() {
            const templateData =
                `Mã sinh viên,Mã nhập học,Họ tên,Ghi chú liên quan đến nợ học phí hoặc nghĩa vụ khác,Ngày sinh,Giới tính,Số CCCD,Ngày cấp,Nơi cấp,Số điện thoại,Nơi sinh,Địa chỉ,Zalo,Số phiếu thu gần nhất,Số hoá đơn gần nhất,Số tiền phải đóng,Số tiền đã thu,Billing(VND),Coll(VND),Discount,LiDo,NgayDangKi,HoTenNguoiThan,MoiQuanHe,SdtNguoiThan,ZaloNguoiThan,EmailNguoiThan,Email,EmailCUSC,password_CUSC,Size,TinhTrangHocTap
21010001,EN2025,Nguyễn Văn A,0,2003-06-12,Nam,123456789,2021-06-01,Cần Thơ,0912345678,Cần Thơ,123 Đường 3/2,0912345678,R12345,I54321,1500000.00,1500000.00,1500000,1500000,0.00,,2025-06-10,Nguyễn Văn B,Cha,0987654321,0987654321,nguoithan@gmail.com,vana@gmail.com,vana@cusc.ctu.edu.vn,matkhau123,M,DangHoc,,`;
    ...
    // 21010002,Trần Thị B,2003-09-15,Nữ,987654321,tranthib@gmail.com,0987654321,Bình Thủy, Cần Thơ`;
        ...
        function downloadTemplate() {
            try {
                const headerRow = [
                    "MaSV", "MaEnroll", "HoTen", "InDebt", "NgaySinh", "GioiTinh", "SoCCCD", "NgayCap", "NoiCap",
                    "Sdt", "NoiSinh", "DiaChi", "Zalo", "Receipt", "Invoice", "Billing", "Coll",
                    "Billing(VND)", "Coll(VND)", "Discount", "LiDo", "NgayDangKi",
                    "HoTenNguoiThan", "MoiQuanHe", "SdtNguoiThan", "ZaloNguoiThan", "EmailNguoiThan",
                    "Email", "EmailCUSC", "password_CUSC", "Size", "TinhTrangHocTap", "created_at", "updated_at"
                ];

                const helperRow = [
                    "Bắt buộc, duy nhất", "Tùy chọn", "Bắt buộc", "Số tiền nợ (VD: 0 hoặc 100000)", "YYYY-MM-DD",
                    "Nam/Nữ (1/0)", "Chỉ số",
                    "YYYY-MM-DD", "Nơi cấp CCCD", "Số điện thoại hợp lệ", "Nơi sinh", "Địa chỉ cư trú", "Zalo cá nhân",
                    "Mã biên nhận", "Mã hóa đơn",
                    "Số tiền phải đóng", "Số tiền đã thu", "Tổng tiền (VNĐ)", "Thu (VNĐ)",
                    "Chiết khấu dạng số thập phân (VD: 0.1)", "Lý do đặc biệt (nếu có)",
                    "Ngày đăng ký", "Tên người thân", "Quan hệ (Cha/Mẹ/...)", "SĐT người thân", "Zalo người thân",
                    "Email người thân",
                    "Email chính", "Email CUSC", "Mật khẩu CUSC", "Size áo đồng phục", "DangHoc/DaTotNghiep/DaNghiHoc",
                    "Auto hoặc để trống", "Auto hoặc để trống"
                ];

                const sampleRow = [
                    "21010001", "EN2025", "Nguyễn Văn A", "0", "2003-06-12", "Nam", "123456789", "2021-06-01",
                    "Cần Thơ",
                    "0912345678", "Cần Thơ", "123 Đường 3/2", "0912345678", "R12345", "I54321", "1500000.00",
                    "1500000.00",
                    "1500000", "1500000", "0.00", "", "2025-06-10", "Nguyễn Văn B", "Cha", "0987654321", "0987654321",
                    "nguoithan@gmail.com", "vana@gmail.com", "vana@cusc.ctu.edu.vn", "matkhau123", "M", "DangHoc", "",
                    ""
                ];

                const rows = [headerRow, helperRow, sampleRow];

                const ws = XLSX.utils.aoa_to_sheet(rows);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, 'Mau_Import_SinhVien');
                XLSX.writeFile(wb, 'mau_import_sinhvien.xlsx');

                iziToast.success({
                    title: 'Thành công',
                    message: 'Đã tải file Excel mẫu đầy đủ thành công!',
                    position: 'topRight'
                });
            } catch (error) {
