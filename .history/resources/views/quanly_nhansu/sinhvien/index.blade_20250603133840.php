                                    ...
                                    <tbody>
                                        @foreach ($sinhViens as $index => $sv)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('student.show', $sv->MaSV) }}" class="btn btn-info btn-sm"
                                                        title="Xem chi tiết">
            ...
            $('a[href="{{ route('dongbo.taikhoan.ldap') }}"]').click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // Ẩn modal loading khi thành công
                        $('#loadingModal').modal('hide');
                        
                        // Hiển thị thông báo kết quả
                        swal({
                            title: 'Thành công',
                            text: response.message || 'Tự tạo tài khoản thành công',
                            icon: 'success'
                        });
                    },
                    error: function(xhr) {
                        // Ẩn modal loading khi có lỗi
                        $('#loadingModal').modal('hide');
                        
                        // Hiển thị thông báo lỗi
                        swal({
                            title: 'Lỗi',
                            text: xhr.responseJSON.message || 'Có lỗi xảy ra khi tự tạo tài khoản',
                            icon: 'error'
                        });
                    }
                });
