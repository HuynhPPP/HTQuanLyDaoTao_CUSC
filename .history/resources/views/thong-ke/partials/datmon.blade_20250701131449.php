...
@push('custom-js')
<script>
    const datMonLabels = {!! json_encode(array_map(fn($m) => \App\Models\MonHoc::find($m)->TenMH ?? $m, array_keys($thongKeDat->toArray()))) !!};
    ...
    new Chart(document.getElementById('chart-datmon'), {
        ...
        options: {
            responsive: true,
            plugins: { legend: { position: 'top' }, title: { display: true, text: 'Tỷ lệ đạt/chưa đạt theo môn học' } },
            scales: {
                y: { beginAtZero: true }
            }
        }
