...
@section('main-content')
    ...
    @if($phuTrach && $phuTrach->CongViecPhuTrach && $phuTrach->MieuTaChiTiet)
        <tr>
            ...
            <td>{{ $phuTrach->MieuTaChiTiet }}</td>
            <td>
                <a href="{{ route('phutrach.edit', $phuTrach->CongViecPhuTrach) }}" class="btn btn-warning btn-sm">
                    <i class="fas fa-edit"></i>
                </a>
                <form action="{{ route('phutrach.destroy', $phuTrach->CongViecPhuTrach) }}" method="POST" style="display:inline-block;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-sm delete-phutrach">
                        <i class="fas fa-trash"></i>
