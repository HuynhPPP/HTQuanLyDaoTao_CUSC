@php
    $userRole = Auth::guard('sinhvien')->check() ? 'student' : 
               (Auth::guard('canbo')->check() ? 'staff' : 
               (Auth::guard('giaovien')->check() ? 'teacher' : 'guest'));
@endphp

<div class="main-sidebar sidebar-style-2" style="position: absolute; top: 0; left: 0;">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand">
            <a href="{{ route('home') }}">CUSC</a>
        </div>
        <div class="sidebar-brand sidebar-brand-sm">
            <a href="{{ route('home') }}">CT</a>
        </div>
        <ul class="sidebar-menu">
            @if(in_array($userRole, ['student', 'staff', 'admin', 'teacher']))
                <li class="menu-header">Trang Chính</li>
                ...
                @if(in_array($userRole, ['student']))
                    ...
                    <li class="{{ request()->routeIs('tracuu.index') ? 'active' : '' }}">
                        <a href="{{ route('tracuu.index') }}" class="nav-link">
                            <i class="fas fa-chart-bar"></i>
                            <span>Tra Cứu Điểm Thi</span>
                        </a>
                    </li>
                    <li class="{{ request()->routeIs('student.calendar.list') ? 'active' : '' }}">
