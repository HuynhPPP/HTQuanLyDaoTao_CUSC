                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <a href="#" id="syncBtn" class="btn btn-success ml-2">
                                <i class="fas fa-sync-alt"></i> Đồng bộ từ AD
                            </a>
                                        ...
                                        @foreach ($accounts as $account)
                                            <tr>
                                                ...
                                                <td class="text-nowrap">
                                                    <form action="{{ route('ad-accounts.destroy', $account->username) }}"
...
@section('custom-js')
    ...
    swal({
        ...
        text: 'Thao tác này sẽ cập nhật dữ liệu từ AD!',
        icon: 'info',
        buttons: ['Hủy', 'Đồng bộ'],
    }).then((willSync) => {
        if (willSync) {
            $.post("{{ route('ad-accounts.sync') }}", {
                _token: '{{ csrf_token() }}'
            }).done(function (response) {
                location.reload();
            }).fail(function () {
                swal('Lỗi', 'Không thể đồng bộ', 'error');
            });
        }
    });
});
