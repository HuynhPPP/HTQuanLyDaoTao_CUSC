                                ...
                                @foreach ($monhocs as $monhoc)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($monhoc->giangViens->count() > 0)
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="dropdown">
                                                        <button class="btn btn-sm btn-outline-info dropdown-toggle" type="button" id="teacherDropdown{{ $monhoc->TenMH }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                            Xem chi tiết
                                                        </button>
                                                        <div class="dropdown-menu" aria-labelledby="teacherDropdown{{ $monhoc->TenMH }}">
                                                            @foreach ($monhoc->giangViens as $gv)
                                                                <div class="dropdown-item-text">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <span>
                                                                            <strong>{{ $gv->MaGV }}</strong> - {{ $gv->HoTenGV }}
                                                                        </span>
                                                                        <div class="btn-group btn-group-sm ml-2" role="group">
                                                                            ...
                                                                            <form action="{{ route('monhoc.remove-teacher', ['tenMH' => $monhoc->TenMH, 'maGV' => $gv->MaGV]) }}" 
                                                                                  method="POST" 
                                        ...
                                        <td>
                                            @php
                                                $loaiTiet = [];
                                                if ($monhoc->TietLT) $loaiTiet[] = 'Lý thuyết';
                                                if ($monhoc->TietTH) $loaiTiet[] = 'Thực hành';
                                                if ($monhoc->TietLTvaTH) $loaiTiet[] = 'LT & TH';
                                            @endphp
                                                ...
                                                @foreach ($loaiTiet as $index => $loai)
                                                    <span class="badge 
                                                        {{ $index == 0 ? 'badge-primary' : 
                                                           ($index == 1 ? 'badge-success' : 'badge-warning') }} 
                                                        mr-1">
                                                        {{ $loai }}
                                                    </span>
                                                @endforeach
