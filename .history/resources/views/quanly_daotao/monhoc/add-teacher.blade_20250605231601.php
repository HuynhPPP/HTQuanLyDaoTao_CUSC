                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Lớp Học</label>
                                        <select name="MaLop[]"
                                            class="form-control @error('MaLop') is-invalid @enderror select2" multiple>
                                            @foreach ($lops as $lop)
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped align-middle">
                    ...
                    <tbody>
                        @foreach ($monhoc->giangViens->groupBy('MaGV') as $maGV => $phanCongList)
                            @php
                                $gv = $phanCongList->first();
                            @endphp
                            <tr>
                                <td>{{ $gv->HoTenGV }}</td>
                                ...
                                {{-- Danh sách lớp --}}
                                <td>
                                    @foreach ($phanCongList as $pc)
                                        <span class="badge badge-info">{{ $pc->MaLop }}</span>
                                    @endforeach
                                </td>

                                {{-- Ngày bắt đầu (hiển thị đầu tiên nếu có nhiều) --}}
                                <td>
                                    @foreach ($phanCongList as $pc)
                                        <div>{{ $pc->NgayBatDau ?? 'Chưa xác định' }}</div>
                                    @endforeach
                                </td>
                                ...
                                <td>
                                    @foreach ($phanCongList as $pc)
                                        <div>{{ $pc->NgayKetThuc ?? 'Chưa xác định' }}</div>
                                    @endforeach
                                </td>
                                ...
                                <td>
                                    <a href="{{ route('monhoc.edit-teacher', ['MaMH' => $monhoc->MaMH, 'maGV' => $maGV]) }}"
                                        class="btn btn-warning btn-sm" data-toggle="tooltip" title="Chỉnh sửa">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('monhoc.remove-teacher', ['MaMH' => $monhoc->MaMH, 'maGV' => $maGV]) }}"
    ...
    <script>
        $(document).ready(function() {
            $('.delete-monhoc').click(function(e) {
                ...
                swal({
                    title: 'Bạn có chắc chắn muốn xóa phân công giảng dạy này?',
