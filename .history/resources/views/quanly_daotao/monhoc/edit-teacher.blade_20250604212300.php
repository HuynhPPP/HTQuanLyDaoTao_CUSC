                        ...
                        <form action="{{ route('monhoc.update-teacher', ['tenMH' => $monhoc->TenMH, 'maGV' => $currentTeacher->MaGV]) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control @error('NgayBatDau') is-invalid @enderror" 
                                       ...
                                       value="{{ old('NgayBatDau', $currentTeacher->giangViens->NgayBatDau) }}">
                                @error('NgayBatDau')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control @error('NgayKetThuc') is-invalid @enderror" 
                                       ...
                                       value="{{ old('NgayKetThuc', $currentTeacher->giangViens->NgayKetThuc) }}">
                                @error('NgayKetThuc')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="GhiChu" name="GhiChu" rows="3">{{ old('GhiChu', $currentTeacher->pivot->GhiChu) }}</textarea>
                            </div>
