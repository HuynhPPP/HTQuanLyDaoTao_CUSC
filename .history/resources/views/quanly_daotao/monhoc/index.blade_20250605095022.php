                                                ...
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="dropdown">
                                                        <button class="btn btn-sm btn-outline-info dropdown-toggle" 
                                                                type="button" 
                                                                id="teacherDropdown{{ $monhoc->MaMH }}"
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="teacherDropdown{{ $monhoc->MaMH }}">
                                                            @foreach ($monhoc->giangViens as $gv)
                                                                <div class="dropdown-item">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <div>
                                                                            <strong>{{ $gv->HoTenGV }}</strong>
                                                                            @if ($gv->pivot->NgayBatDau || $gv->pivot->NgayKetThuc)
                                                                                <small class="d-block text-muted">
                                                                                    {{ $gv->pivot->NgayBatDau ? 'Từ: ' . $gv->pivot->NgayBatDau : '' }}
                                                                                    {{ $gv->pivot->NgayKetThuc ? 'Đến: ' . $gv->pivot->NgayKetThuc : '' }}
                                                                                </small>
                                                                            @endif
                                                                        </div>
                                                            ...
                                                            <a href="{{ route('monhoc.add-teacher', $monhoc->MaMH) }}" 
                                                               class="dropdown-item text-primary">
                                                                <i class="fas fa-plus-circle mr-2"></i>Thêm Giảng Viên
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            @else
                                                <a href="{{ route('monhoc.add-teacher', $monhoc->MaMH) }}"
                                                   class="btn btn-outline-info btn-sm">
                                                    <i class="fas fa-user-plus mr-1"></i>Phân Công Giảng Viên
                                                </a>
                                            @endif
                                        </td>
                                        <td>{{ $monhoc->GioGoc }}</td>
