                        ...
                        <form action="{{ route('lophoc.update', $lophoc->MaLop) }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="MaLop" class="form-label">Mã lớp <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="MaLop" name="MaLop"
                                        value="{{ old('MaLop', $lophoc->MaLop) }}">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="TenLop" class="form-label">Tên lớp <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control @error('TenLop') is-invalid @enderror"
                                        id="TenLop" name="TenLop" value="{{ old('TenLop', $lophoc->TenLop) }}">
                                    @error('TenLop')
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control @error('NgayBatDau') is-invalid @enderror"
                                        ...
                                        value="{{ old('NgayBatDau', $lophoc->NgayBatDau) }}">
                                    @error('NgayBatDau')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control @error('MaChuongTrinh') is-invalid @enderror"
                                        ...
                                        @foreach ($chuongtrinhs as $chuongtrinh)
                                            ...
                                            <option value="{{ $chuongtrinh->MaChuongTrinh }}"
                                                {{ old('MaChuongTrinh', $chuongtrinh->MaChuongTrinh) == $chuongtrinh->MaChuongTrinh ? 'selected' : '' }}>
                                                {{ $phong->TenPhong }} ({{ $phong->LoaiPhong }})</option>
                                        @endforeach
                                    </select>
                                    @error('MaChuongTrinh')
                                        <div class="invalid-feedback">{{ $message }}</div>
