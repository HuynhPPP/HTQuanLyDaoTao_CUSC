...
@section('main-content')
    <div class="section">
        <div class="section-header">
            <h1>Danh sách sinh viên lớp "{{ $lophoc->MaLop }} - {{ $lophoc->TenLop }}"</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="{{ route('about') }}">Trang chủ</a></div>
                <div class="breadcrumb-item active"><a href="{{ route('lophoc.index') }}">Danh sách lớp học</a></div>
                <div class="breadcrumb-item">Danh sách sinh viên lớp "{{ $lophoc->MaLop }} - {{ $lophoc->TenLop }}</div>
            </div>
        </div>
                                    ...
                                    <tbody>
                                        @forelse($lophoc->danhSachSinhVien as $dssv)
                                            <tr>
                                                ...
                                                <td>
                                                    <form
                                                        action="{{ route('lophoc.remove-student', ['malop' => $lophoc->MaLop, 'masv' => $dssv->MaSV]) }}"
                                                        method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="btn btn-danger btn-sm delete-sinhvien">
