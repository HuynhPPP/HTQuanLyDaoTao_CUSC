                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('saveSchedule') }}" onsubmit="return validateForm()">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="KhoaDaoTao" class="form-control @error('KhoaDaoTao') is-invalid @enderror"
                                    name="KhoaDaoTao">
                                    <option value="">----- Khóa Đào Tạo -----</option>
                                    @foreach ($khoadaotaos as $khoadaotao)
                                        <option value="{{ $khoadaotao->TenKhoaDaoTao }}">{{ $khoadaotao->TenKhoaDaoTao }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('KhoaDaoTao')
                            ...
                            <div class="mb-3">
                                ...
                                <select id="ChuongTrinhTrienKhai"
                                    class="form-control @error('ChuongTrinhTrienKhai') is-invalid @enderror"
                                    name="ChuongTrinhTrienKhai">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="Lop" class="form-control @error('Lop') is-invalid @enderror"
                                    name="Lop">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="HocKi" class="form-control @error('HocKi') is-invalid @enderror"
                                    name="HocKi">
        ...
        <div class="row justify-content-center mb-5">
            <div class="col-lg-8">
                <div class="list-group">
                    @forelse ($tkbs as $tkb)
                        <a href="{{ route('schedule.detail', $tkb->TenTKB) }}"
                            class="list-group-item list-group-item-action fs-5 py-3">
                            {{ $tkb->TenTKB }}
                        </a>
                    @empty
    ...
    <script>
        document.getElementById('KhoaDaoTao').addEventListener('change', function() {
            ...
            if (KhoaDaoTao && KhoaDaoTao !== "") {
                fetch(`schedules/getChuongTrinh/${(KhoaDaoTao)}`)
                    .then(response => {
                        if (!response.ok) {
        ...
        document.getElementById('ChuongTrinhTrienKhai').addEventListener('change', function() {
            ...
            if (chuongTrinh && chuongTrinh !== "") {
                fetch(`schedules/getLop/${chuongTrinh}`)
                ...
                fetch(`schedules/getHK/${chuongTrinh}`)
                    .then(response => response.json())
                    .then(data => {
                        HocKiSelect.innerHTML = '<option value="">----- Mã Học Kỳ -----</option>';
                        data.forEach(HocKi => {
                            HocKiSelect.innerHTML +=
                                `<option value="${HocKi.MaHK}">${HocKi.MaHK} ${HocKi.TenHK}</option>`;
                        });
