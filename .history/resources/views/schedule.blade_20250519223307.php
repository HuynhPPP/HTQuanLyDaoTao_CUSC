...
@section('main-content')
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <strong>Mã lớp:</strong> {{ $schedule->MaLop }}
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        <strong>Bắt đầu học từ ngày:</strong> 
                                        {{ \Carbon\Carbon::parse($schedule->NgayHoc)->format('d/m/Y') }}
                                    </div>
                                    <div class="mb-2">
                                        <strong>Học Lý thuyết tại phòng:</strong>
                                        {{ $phonglt->TenPhong ?? ' Chưa có ' }}
                                    </div>
                                    <div class="mb-2">
                                        <strong>Học Thực hành tại phòng:</strong>
                                        {{ $phongth->TenPhong ?? ' Chưa có ' }}
                                    </div>
                                </div>
                            </div>

                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="table-schedule">
                                    <thead>
                                        <tr>
                                            <th>Ngày</th>
                                            <th>Tuần</th>
                                    ...
                                    <tbody>
                                        @foreach ($scheduleMatrix as $week => $days)
                                            @php $weekDates = collect($days)->pluck('date')->toArray(); @endphp
                                            <tr>
                                                <td>{{ implode(' - ', [$weekDates[0], end($weekDates)]) }}</td>
                                                <td class="text-center">{{ $week }}</td>
                                                <td>{{ $dsmh->TenKhungGio ?? '' }}</td>
                                                @foreach ($days as $dayData)
                                                    <td style="{{ $dayData['style'] }}">
                                                        {{ $dayData['subject'] }}
                                                    </td>
                                                @endforeach
                                            </tr>
    ...
    <!-- Modal Tự học -->
    <div class="modal fade" id="SelfStudyModal" tabindex="-1" role="dialog" aria-labelledby="SelfStudyModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <!-- Modal Chỉnh sửa TKB -->
    <div class="modal fade" id="EditTKBModal" tabindex="-1" role="dialog" aria-labelledby="EditTKBModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="{{ route('EditTKB', $schedule->TenTKB) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="NgayHoc" name="NgayHoc" value="{{ $schedule->NgayHoc }}" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
