...
Route::middleware([RoleMiddleware::class . ':teacher'])->group(function () {
    Route::get('/giao-vien/ho-so', [GiaoVienController::class, 'profile'])->name('about');
});

// Nhóm route cho giáo viên, chỉ cho phép role teacher
Route::middleware([RoleMiddleware::class . ':teacher'])->group(function () {
    Route::get('/giao-vien/ho-so', GiaoVienController::class, 'profile'])
        ->name('giaovien.profile');
    
    Route::post('/giao-vien/ho-so/cap-nhat', [GiaoVien\ProfileController::class, 'updateProfile'])
