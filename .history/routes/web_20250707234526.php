...
use App\Http\Controllers\Backend\AccountAuthenticationController;
use App\Http\Controllers\CaptchaController;
use App\Http\Controllers\ThongKe\ThongKeDashboardController;
use App\Http\Controllers\ThongKe\ThongKeBaoCaoDoAnController;
...
use App\Http\Controllers\ToChucThi\LapBangDiemThiController;
use App\Http\Controllers\ToChucThi\SinhVienDuThiController;
use App\Http\Controllers\Schedules\SchedulesController;
use App\Http\Controllers\Front\SinhVien\TraCuuDiemThiController;
use App\Http\Controllers\Front\SinhVien\CalendarController;
...
Route::post('ldap', [AccountAuthenticationController::class, 'authentication'])->name('ldap');
Route::get('/logout', [AccountAuthenticationController::class, 'logout'])->name('logout');
...
Route::middleware([RoleMiddleware::class . ':admin'])->group(function () {
    ...
    Route::prefix('schedules')->group(function () {
        ...
        Route::get('/ministry/schedule/{TenTKB}', [SchedulesController::class, 'scheduleDetail'])->name('schedule.detail');
        Route::get('/teacher-schedule/{TenTKB}', [SchedulesController::class, 'teacherSchedule'])->name('teacherSchedule');
        Route::delete('/ministry/schedules/schedule/{TenTKB}', [SchedulesController::class, 'deleteSchedule'])->name('deleteSchedule');
        Route::get('/ministry/schedules/export-excel/{TenTKB}', [SchedulesController::class, 'exportExcel'])->name('exportExcel');
        Route::post('/saveholiday/{TenTKB}', [SchedulesController::class, 'saveholiday'])->name('saveholiday');
        Route::get('/get-subjects', [SchedulesController::class, 'getSubjects'])->name('getSubjects');
        Route::post('/ministry/schedules/update-subjects/{TenTKB}', [SchedulesController::class, 'updateScheduleSubjects'])->name('updateScheduleSubjects');
        Route::post('/saveSelfStudy/{TenTKB}', [SchedulesController::class, 'saveSelfStudy'])->name('saveSelfStudy');
        Route::get('/export-teacher-schedule/{TenTKB}', [SchedulesController::class, 'exportTeacherSchedule'])->name('exportTeacherSchedule');
        Route::get('/getChuongTrinh/{TenKhoaDaoTao}', [SchedulesController::class, 'getChuongTrinh']);
        Route::get('/getLop/{MaChuongTrinh}', [SchedulesController::class, 'getLop']);
        Route::get('/getHK/{MaChuongTrinh}', [SchedulesController::class, 'getHK']);
        Route::post('/saveTimeSlot/{TenTKB}', [SchedulesController::class, 'saveTimeSlot'])->name('saveTimeSlot');
        Route::post('/EditTKB/{TenTKB}', [SchedulesController::class, 'EditTKB'])->name('EditTKB');
    });
    Route::prefix('student')->group(function () {
        ...
        Route::post(
            '/ldap/account/send-bulk',
            [SinhVienController::class, 'guiThongTinTaiKhoanHangLoat']
        )
            ->name('ldap.account.send.bulk');
    });
    ...
    Route::prefix('giaovien')->group(function () {
        ...
        Route::post(
            '/ldap/account/send-bulk',
            [GiaoVienController::class, 'guiThongTinTaiKhoanHangLoat']
        )
            ->name('giaovien.ldap.account.send.bulk');
    ...
    Route::prefix('chuongtrinh')->group(function () {
        ...
        Route::delete('/monhoc/{maChuongTrinh}/{MaMH}', [ChuongTrinhDaoTaoController::class, 'destroyMonHoc'])->name('chuongtrinh.monhoc.destroy');

        Route::post('/hinh-thuc-danh-gia/xoa/{id}/{maChuongTrinh}', [ChuongTrinhDaoTaoController::class, 'xoaHinhThucDanhGia'])->name('chuongtrinh.xoa.hinhthucdanhgia');
        Route::post('/tieu-chi-xep-loai/xoa', [ChuongTrinhDaoTaoController::class, 'xoaTieuChiXepLoai'])->name('chuongtrinh.xoa.tieuchixeploai');
    });
    ...
    Route::prefix('thong-ke')->name('thong-ke.')->group(function () {
        // Thống kê dashboard
        Route::get('/thong-ke-dashboard', [ThongKeDashboardController::class, 'index'])
            ->name('dashboard');
        Route::get('/thong-ke/hoc-luc', [ThongKeDashboardController::class, 'chonChuongTrinhLopHoc'])
            ->name('thong-ke-hoc-luc');
        Route::get('/tong-ket-hoc-luc/{MaLop}/{MaChuongTrinh}', [ThongKeDashboardController::class, 'thongkehoctap'])
...
Route::middleware([RoleMiddleware::class . ':admin,staff'])->group(function () {
    ...
    Route::prefix('tochucthi')->group(function () {
        ...
        Route::get('/chon-lop-mon-hoc', [LapBangDiemThiController::class, 'chonLopVaMonHoc'])->name('lapbangdiemchitiet.chon-lop-mon-hoc');
        Route::get('/get-mon-hoc-theo-lop', [LapBangDiemThiController::class, 'getMonHocTheoLop'])->name('lapbangdiemchitiet.chon-mon-hoc-theo-lop');
        Route::get('/bang-diem-chi-tiet/{maLop}/{maMH}', [LapBangDiemThiController::class, 'bangDiemChiTiet'])->name('bang-diem-chi-tiet');
        Route::get('/xuat-bang-diem/{maLop}/{maMH}', [LapBangDiemThiController::class, 'xuatBangDiemChiTiet'])
            ->name('diemthi.xuat-bang-diem-chi-tiet');

        Route::get('/chon-chuong-trinh-bang-diem-tong', [LapBangDiemThiController::class, 'chonChuongTrinh'])->name('chon-chuong-trinh-bang-diem-tong');
        Route::get('/bang-diem-tong/{maChuongTrinh}', [LapBangDiemThiController::class, 'bangDiemTong'])->name('bang-diem-tong');

        Route::get(
            '/thong-ke-sinh-vien-dat-mon/{maChuongTrinh}',
            [LapBangDiemThiController::class, 'thongKeSinhVienDatMon']
        )
...
Route::middleware([RoleMiddleware::class . ':teacher'])->group(function () {
    Route::get('/giao-vien/ho-so', [MainTeacherController::class, 'profile'])->name('giaovien.profile');
    Route::post('/giao-vien/ho-so/cap-nhat', [MainTeacherController::class, 'updateProfile'])
        ->name('giaovien.profile.update');
    Route::post('/giao-vien/doi-mat-khau', [MainTeacherController::class, 'changePassword'])
        ->name('giaovien.change.password');

    // Lịch coi thi
    Route::prefix('giao-vien/lich-thi')->group(function () {
        Route::get('/', [CalendarExamController::class, 'index'])
            ->name('giaovien.lichthi.index');
        Route::get('/{maLichThi}', [CalendarExamController::class, 'chiTietLichThi'])
            ->name('giaovien.lichthi.chi-tiet');
    });

    // Nhập điểm thi
    Route::prefix('giao-vien/nhap-diem-thi')->group(function () {
        Route::get('/', [AddPointExamController::class, 'danhSachLopDay'])
            ->name('giaovien.nhapdiemthi.danh-sach-mon');
        Route::post('/luu-diem/{MaLopHoc}/{MaMH}', [AddPointExamController::class, 'luuDiem'])
...
Route::middleware([RoleMiddleware::class . ':student'])->group(function () {
    Route::get('/sinh-vien/ho-so', [MainStudentController::class, 'profile'])->name('student.profile');
    Route::post('/sinh-vien/ho-so/cap-nhat', [MainStudentController::class, 'updateProfile'])
        ->name('student.profile.update');
    Route::post('/sinh-vien/doi-mat-khau', [MainStudentController::class, 'changePassword'])
