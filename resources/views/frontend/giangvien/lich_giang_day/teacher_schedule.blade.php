...
@section('main-content')
    <section class="section">
        ...
        @if (isset($schedule))
            <div class="mb-3">
                <strong>Giảng viên:</strong> {{ $giangVien->HoTenGV ?? '-' }}<br>
                <strong>Lớp:</strong>
                <form method="get" class="d-inline">
                    <select name="lop" onchange="this.form.submit()" class="mr-2">
                        @foreach($maLops as $lop)
                            <option value="{{ $lop }}" {{ $selectedMaLop == $lop ? 'selected' : '' }}>{{ $lop }}</option>
                        @endforeach
                    </select>
            ...
            <form method="get" class="mb-3 d-flex align-items-center gap-2">
                <input type="hidden" name="lop" value="{{ $selectedMaLop }}">
                <label for="week" class="mb-0 mr-2">Chọn tuần:</label>
                <select name="week" id="week" class="mr-4" onchange="this.form.submit()" {{ request('viewMode', 'week') == 'all' ? 'disabled' : '' }}>
                        ...
                        @foreach ($weeksToShow as $week)
                            @if (isset($scheduleMatrix[$week]))
                                @php
                                    ...
                                    $lastDay = $scheduleMatrix[$week][$weekDays[count($weekDays) - 1]]['date'] ?? '';
                                @endphp
                                <tr>
                                        ...
                                        <td class="align-middle text-center" style="{{ $cell['style'] ?? '' }}">
                                            @if ($cell)
                                                @php
                                                    ...
                                                    $tenMH = null;
                                                    $mh = null;
                                                    if (isset($subjectOccurrences) && isset($subjectOccurrences[$subject])) {
                                                    ...
                                                    if (isset($phongHocTheoNgay) && isset($cell['date'])) {
                                                        $ngayCell = \Carbon\Carbon::createFromFormat('d/m/Y', $cell['date'])->format('Y-m-d');
                                                        $phong = $phongHocTheoNgay[$ngayCell] ?? $phongHocTheoNgay->first();
                                                    }
                                                    // Lấy thời gian học
                                                    $thoiGian = $dsmh->khungGio->ThoiGian ?? ($dsmh->TenKhungGio ?? '-');
                                                    // Lấy loại tiết học
                                                    $loaiTiet = '-';
                                                    if(isset($mh) && $mh->monhoc) {
                                                        $types = [];
                                                        if(!empty($mh->monhoc->TietLT)) $types[] = 'Lý thuyết';
                                                        if(!empty($mh->monhoc->TietTH)) $types[] = 'Thực hành';
                                                        if(!empty($mh->monhoc->TietLTvaTH)) $types[] = 'LT & TH';
                                                        if(count($types)) $loaiTiet = implode(', ', $types);
                                                    }
                                                @endphp
                                                <div>
                                                ...
                                                @if (
                                                    ...
                                                    <div class="small text-muted">
                                                        ...
                                                        <i class="fas fa-book-open"></i> Loại tiết: {{ $loaiTiet }}
                                                    </div>
                                                @endif
                                            @else
                                                -
